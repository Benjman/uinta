...
void initPawns(Pawns& pawns, const IntersectionBuildings& buildings, const PawnConfig& config, size_t count) noexcept {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    pawns.add(startPos, config.walkingSpeed, initialHunger, initialEnergy, initialMoney, home, job);
  }
}
