...
void updatePawnNeeds(Pawns& pawns, const PawnConfig& config, f32 simHours) noexcept {
...
PawnState decidePawnState(const Pawns& pawns, const IntersectionBuildings& buildings, const SimTime& time,
                          const PawnConfig& config, size_t i) noexcept {
  ...
  if (hunger > config.criticalHungerThreshold) {
    ...
    BuildingId shop = buildings.findNearestByType(BuildingType::Shop, pawns.positions[i]);
    if (currentBldg == shop) {
...
BuildingId determineDestination(const Pawns& pawns, const IntersectionBuildings& buildings, const SimTime& time,
                                const PawnConfig& config, size_t i) noexcept {
  f32 hunger = pawns.hunger[i];
  f32 energy = pawns.energy[i];
  BuildingId job = pawns.jobs[i];
...
void transitionPawnState(Pawns& pawns, IntersectionBuildings& buildings, const SimTime& time, const PawnConfig& config,
...
void pawnDecisionSystem(Pawns& pawns, IntersectionBuildings& buildings, const SimTime& time, const PawnConfig& config,
                        f32 dt) noexcept {
    ...
    switch (currentState) {
      ...
      case PawnState::Working:
        ...
        if (!time.isWorkHours(config.workStartHour, config.workEndHour) ||
            pawns.hunger[i] > config.criticalHungerThreshold || pawns.energy[i] < config.criticalEnergyThreshold) {
          shouldDecide = true;
        }
        break;
    }
