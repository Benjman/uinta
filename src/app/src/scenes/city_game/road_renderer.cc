...
RoadRenderer::RoadRenderer(BasicShaderManager* bsm, const OpenGLApi* gl) noexcept
...
void RoadRenderer::addRoad(const Road& road) noexcept {
  ...
  transform = glm::translate(transform, glm::vec3(road.position.x + 0.05f, 0.0f, road.position.y - 0.05f));
