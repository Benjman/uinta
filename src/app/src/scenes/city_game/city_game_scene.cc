...
void CityGameScene::onDebugUi() noexcept {
  ...
  // Time display
  ImGui::Text("Day %d, %02d:%02d", simTime_.day, simTime_.hourInt(), simTime_.minuteInt());
  ...
  if (ImGui::CollapsingHeader("Pawn Details")) {
    for (size_t i = 0; i < pawns_.count(); ++i) {
      ...
      const char* location = pawns_.currentBuilding[i] == NULL_BUILDING ? "Outside" : "Inside";

      ImGui::Text("Pawn %zu [%s] - %s", i, pawnStateName(pawns_.states[i]), location);
      ImGui::Text("  Hunger: %.0f%%  Energy: %.0f%%  Money: $%.0f", pawns_.hunger[i] * 100.0f,
                  pawns_.energy[i] * 100.0f, pawns_.money[i]);
  ...
  if (ImGui::CollapsingHeader("Buildings")) {
    for (size_t i = 0; i < buildings_.count(); ++i) {
      ...
      ImGui::Text("%s: %d/%d occupants", typeName, buildings_.currentOccupants[i], buildings_.maxOccupants[i]);
    }
  }
}
...
glm::vec3 CityGameScene::calculateSunDirection(f32 hour) const noexcept {
  ...
  if (hour < sunriseHour || hour > sunsetHour) {
    ...
    if (hour < sunriseHour) {
      ...
      f32 nightProgress = (hour - sunsetHour) / (24.0f - sunsetHour);  // 0 at sunset, 1 at
                                                                       // midnight
...
void CityGameScene::updateLighting() noexcept {
  ...
  if (simTime_.hour >= sunriseHour && simTime_.hour <= sunsetHour) {
    ...
    f32 dayProgress = (simTime_.hour - sunriseHour) / (sunsetHour - sunriseHour);
    // Peak at noon
  ...
  if (simTime_.hour >= 6.0f && simTime_.hour < 7.0f) {
    ...
    lightColor = glm::mix(glm::vec3(1.0f, 0.6f, 0.4f), glm::vec3(1.0f, 0.95f, 0.9f), t);
  } else if (simTime_.hour >= 17.0f && simTime_.hour < 18.0f) {
    f32 t = simTime_.hour - 17.0f;  // 0 to 1
    lightColor = glm::mix(glm::vec3(1.0f, 0.95f, 0.9f), glm::vec3(1.0f, 0.6f, 0.4f), t);
  }
