...
PlacementInputHandler::PlacementInputHandler(Scene* scene) noexcept {
  ...
  cursorManager_ = engine->service<CursorManager>();
  ...
  inputSystem->subscribeKey(Key::Num3 | Action::Press, [&](Key, Action, Mod) { mode_ = PlacementMode::Road; });

  // Mouse click: placement request
  inputSystem->subscribeMouse(MouseBtn::Left | Action::Release, [&](auto, auto, auto) {
    auto worldPos = cursorManager_->worldPoint();
