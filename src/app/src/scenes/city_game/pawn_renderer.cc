...
PawnRenderer::PawnRenderer(BasicShaderManager* bsm, const OpenGLApi* gl) noexcept : bsm_(bsm), gl_(gl) {
  initStateMeshes();
}
...
void PawnRenderer::render(const Pawns& pawns) noexcept {
  ...
  for (size_t i = 0; i < pawns.count(); ++i) {
    ...
    bsm_->drawElements(GL_TRIANGLES, static_cast<GLsizei>(cubeIndexCount_), GL_UNSIGNED_INT, nullptr);
  }
