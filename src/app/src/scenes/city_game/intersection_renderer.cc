...
IntersectionRenderer::IntersectionRenderer(BasicShaderManager* bsm, const OpenGLApi* gl) noexcept
    : buildingVbo_(GL_ARRAY_BUFFER, 0, gl), groundVbo_(GL_ARRAY_BUFFER, 0, gl), bsm_(bsm) {
...
void IntersectionRenderer::initGroundMesh() noexcept {
  ...
  vertices.push_back(Vertex{.position = {-halfSize, 0, -halfSize}, .normal = normal, .color = groundColor});
  vertices.push_back(Vertex{.position = {halfSize, 0, -halfSize}, .normal = normal, .color = groundColor});
  vertices.push_back(Vertex{.position = {halfSize, 0, halfSize}, .normal = normal, .color = groundColor});
  vertices.push_back(Vertex{.position = {-halfSize, 0, halfSize}, .normal = normal, .color = groundColor});
  ...
  groundVtxSegment_ = groundVbo_.bufferData(vertices.data(), bufSize, GL_STATIC_DRAW, groundVtxSegment_);
...
void IntersectionRenderer::buildMesh(const IntersectionBuildings& buildings) noexcept {
  ...
  auto bufSize = combined.vertices().size() * Vertex::ElementCount * sizeof(f32);
  buildingVtxSegment_ =
      buildingVbo_.bufferData(combined.vertices().data(), bufSize, GL_STATIC_DRAW, buildingVtxSegment_);

  VaoGuard vag(&buildingVao_);
  buildingIdxSegment_ = buildingVao_.ebo(combined.elements(), buildingIdxSegment_);
...
void IntersectionRenderer::render() const noexcept {
  ...
  if (groundIndexCount_ > 0) {
    ...
    bsm_->drawElements(GL_TRIANGLES, static_cast<GLsizei>(groundIndexCount_), GL_UNSIGNED_INT, nullptr);
  }
  ...
  if (buildingIndexCount_ > 0) {
    ...
    bsm_->model(glm::mat4(1.0f));
    bsm_->drawElements(GL_TRIANGLES, static_cast<GLsizei>(buildingIndexCount_), GL_UNSIGNED_INT, nullptr);
  }
