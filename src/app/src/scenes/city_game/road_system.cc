...
bool RoadSystem::hasRoad(glm::ivec2 position) const noexcept { return roads_.find(position) != roads_.end(); }
...
void RoadSystem::updateConnections(glm::ivec2 position) noexcept {
  ...
  connectNeighbor(position, position + glm::ivec2(0, 1), RoadConnection::North, RoadConnection::South);
  connectNeighbor(position, position + glm::ivec2(0, -1), RoadConnection::South, RoadConnection::North);
  connectNeighbor(position, position + glm::ivec2(1, 0), RoadConnection::East, RoadConnection::West);
  connectNeighbor(position, position + glm::ivec2(-1, 0), RoadConnection::West, RoadConnection::East);
}

void RoadSystem::connectNeighbor(glm::ivec2 pos, glm::ivec2 neighborPos, RoadConnection outgoing,
  ...
  if (std::find(lastUpdated_.begin(), lastUpdated_.end(), neighborPos) == lastUpdated_.end()) {
