...
bool isInsideBuilding(const glm::vec3& pos, const IntersectionBuildings& buildings, size_t b) {
  glm::vec3 center = buildings.positions[b];
  glm::vec3 halfDims = buildings.dimensions[b] * 0.5f;

  return pos.x >= center.x - halfDims.x && pos.x <= center.x + halfDims.x && pos.z >= center.z - halfDims.z &&
         pos.z <= center.z + halfDims.z;
}

// Push position out of buildings
glm::vec3 resolveCollisions(glm::vec3 pos, const IntersectionBuildings& buildings) {
  for (size_t b = 0; b < buildings.count(); ++b) {
    ...
    f32 minPushX = (std::abs(pushLeft) < std::abs(pushRight)) ? pushLeft : pushRight;
    f32 minPushZ = (std::abs(pushBack) < std::abs(pushFront)) ? pushBack : pushFront;
...
void movementSystem(Pawns& pawns, IntersectionBuildings& buildings, f32 dt) noexcept {
  for (size_t i = 0; i < pawns.count(); ++i) {
    ...
    if (dist < ARRIVAL_THRESHOLD) {
      ...
      if (arrivedAt != NULL_BUILDING) {
        ...
        } else if (type == BuildingType::Residential && pawns.homes[i] == arrivedAt) {
