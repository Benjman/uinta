...
Mesh generateHouseMesh(Color color, const glm::mat4& transform) noexcept {
  ...
  for (size_t i = 0; i < mMesh.NumVert(); ++i) {
    ...
    vertices.emplace_back(
        Vertex{.position = position, .normal = normal, .color = color, .uv = Vertex::uv_type{0.0f, 0.0f}});
  }
...
CityRenderer::CityRenderer(BasicShaderManager* bsm, const OpenGLApi* gl) noexcept
...
void CityRenderer::addBuilding(const Building& building) noexcept {
  ...
  glm::mat4 transform = glm::translate(glm::mat4(1), glm::vec3(building.position.x, 0.00, building.position.y));
  f32 randomAngle = glm::radians<f32>(static_cast<f32>(absl::Uniform(bitgen, 0.0, 360.0)));
