...
class TextScene : public Scene {
 public:
  explicit TextScene(Scene* parent) noexcept
      : Scene(parent, SceneLayer::UI),
        font_("font/JetBrains-Mono-Nerd-Font-Complete.ttf"),
        shader_(engine()->gl()),
        vbo_(GL_ARRAY_BUFFER, 0, engine()->gl()) {
    if (!font_.status().ok()) {
      LOG(FATAL) << font_.status().message();
    }

    const auto* message = "Translation - edsf\nYaw - wr\nPitch - qa\nDist - cv";

    auto status = TextMeshBuilder(&font_, message)
    ...
    const auto& data = status.value();

    VboGuard vbg(&vbo_);
    vbo_.bufferData(data.data(), static_cast<GLsizei>(data.size() * sizeof(f32)), GL_STATIC_DRAW);
  ...
  void render(time_t /*unused*/) noexcept override {
    ...
    engine()->gl()->drawArrays(GL_TRIANGLES, drawStart, static_cast<GLsizei>(drawCount_));
  }
