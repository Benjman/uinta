...
class RoadSystem {
 public:
  ...
  // Access
  const std::map<glm::ivec2, Road, GridPosCompare>& roads() const noexcept { return roads_; }

  // Returns list of positions whose connections were updated
  const std::vector<glm::ivec2>& getLastUpdatedRoads() const noexcept { return lastUpdated_; }
 ...
 private:
  ...
  void connectNeighbor(glm::ivec2 pos, glm::ivec2 neighborPos, RoadConnection outgoing,
