...
struct SimTime {
  ...
  inline f32 simHoursPerRealSecond() const noexcept { return BASE_SIM_HOURS_PER_REAL_SECOND * timeScale; }
  ...
  i32 minuteInt() const noexcept { return static_cast<i32>((hour - static_cast<f32>(hourInt())) * 60.0f); }

  // Check if current time is within work hours
  bool isWorkHours(f32 startHour, f32 endHour) const noexcept { return hour >= startHour && hour < endHour; }
};
