...
class TreeScene : public Scene {
 public:
  ...
  void generateTrees() noexcept {
    absl::BitGen bitgen;
    absl::uniform_real_distribution<f32> random(-distrobutionSize_.x * 0.5f,
                                                distrobutionSize_.y * 0.5f);
  ...
  void addTree(glm::mat4 transform) noexcept {
    ...
    VboGuard vbg(&vbo_);
    vtxSegment_ = vbo_.bufferData(data, static_cast<GLsizei>(size),
                                  GL_STATIC_DRAW, vtxSegment_);
 ...
 private:
  struct Flags final {
    ...
    [[nodiscard]] bool isSway() const noexcept {
      return (flags_ & SwayMask) != 0;
    }
  ...
  i32 treeCount() const noexcept {
    return static_cast<i32>(
        std::round(treesPerMeter_ * distrobutionSize_.x * distrobutionSize_.y));
  }
};
