...
class CameraSceneShader : public Shader {
 public:
  explicit CameraSceneShader(const OpenGLApi* gl) noexcept
      : Shader({{GL_VERTEX_SHADER, "shader/camera.vs.glsl"}, {GL_FRAGMENT_SHADER, "shader/camera.fs.glsl"}}, gl) {
...
class CameraScene : public Scene {
 public:
  explicit CameraScene(Scene* parent) noexcept
    ...
    assert(camera_);
    camera_->addListener<CameraEvent::ViewMatrixUpdated>([&](const ViewMatrixUpdateEvent& e) {
      ShaderGuard guard(&shader_);
      shader_.view = e.view;
    });
  ...
  void render(time_t /*unused*/) noexcept override {
    ...
    gl_->drawElements(GL_TRIANGLES, static_cast<GLsizei>(indexCount_), GL_UNSIGNED_INT, nullptr);
  }
