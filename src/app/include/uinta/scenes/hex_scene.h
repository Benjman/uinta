...
class HexScene final : public Scene {
 public:
  ...
  void generateMesh() noexcept {
    ...
    HexGroup group(origin, radius, static_cast<f32>(hexagonSize), static_cast<f32>(fillPercentage));
    ...
    auto offset = vbo_.bufferData(fill.vertices().data(),
                                  static_cast<GLsizeiptr>(fill.vertices().size() * Vertex::ElementCount * sizeof(f32)),
                                  GL_STATIC_DRAW);

    offset = vbo_.bufferData(outline.vertices().data(),
                             static_cast<GLsizeiptr>(outline.vertices().size() * Vertex::ElementCount * sizeof(f32)),
  ...
  void preRender(time_t /*unused*/) noexcept override {
    if (input_->isMouseReleased(MOUSE_BUTTON_LEFT)) {
      ...
      auto hex = group_.pointToHex(cursorManager->worldPoint().xz());
  ...
  void onDebugUi() noexcept override {
    ...
    if (ImGui::BeginCombo("Layout", group_.flags.isLayoutHexagon() ? HexStr : RhombusStr)) {
    ...
    update |= ImGui::SliderFloat("##hex_radius", &group_.size, 0.001, 4, "Radius %.2f");
    update |= ImGui::SliderFloat("##hex_fill", &group_.fillPercentage, 0, 1, "Fill %.2f");
    update |= ImGui::SliderFloat("##hex_linewidth", &lineWidth_, 0.01, 12, "Line Width %.2f");
    ImGui::Separator();
    if (update) {
  ...
  void render(time_t /*unused*/) noexcept override {
    ...
    for (auto i = 0; i < hexCount_; i++) {
      ...
      if ((flags & DrawOutlineMask) != 0u) {
        bsm_->drawArrays(GL_LINE_STRIP, static_cast<GLint>(outlineOffset_ + static_cast<GLint>(OutlineIndicies * i)),
                         OutlineIndicies);
      }
    }
  }
