...
class Monitor {
 public:
  struct Flags final {
    ...
    [[nodiscard]] bool isPrimary() const noexcept { return (flags_ & PrimaryMask) != 0; }
    ...
    [[nodiscard]] bool isConnected() const noexcept { return (flags_ & ConnectionMask) != 0; }
  ...
  Monitor(std::string_view name = "", i32 width = 0, i32 height = 0, i32 hz = 0, void* userData = nullptr,
          bool isPrimary = false, bool isConnected = true)
      : name_(name), width_(width), height_(height), hz_(hz), userData_(userData) {
