...
class Platform {
 public:
  ...
  Monitor* addMonitor(const Monitor& monitor) noexcept { return &monitors_.emplace_back(monitor); }

  std::span<const Monitor> monitors() const noexcept { return monitors_; }

  std::optional<Monitor*> findMonitor(void* monitorPtr) noexcept {
    const auto& itr = std::ranges::find_if(monitors_.begin(), monitors_.end(),
                                           [monitorPtr](auto& monitor) { return monitor.userData() == monitorPtr; });
    if (itr == monitors_.end()) {
      return std::nullopt;
    }
    return &(*itr);
  }

  std::optional<const Monitor*> primaryMonitor() const noexcept {
    for (const auto& monitor : monitors_) {
      if (monitor.flags().isPrimary()) {
        return &monitor;
      }
    }
    return std::nullopt;
  ...
  virtual void makeFullscreen(bool makeFullscreen, Window*, const Monitor* = nullptr) noexcept = 0;
  ...
  void engine(Engine* engine) noexcept {
    assert(engine_ == nullptr && "Unexpected attempt to overwrite engine pointer");
    engine_ = engine;
  }
