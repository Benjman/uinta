...
class Input final {
 public:
  struct Flags final {
    ...
    [[nodiscard]] bool isKeyDown() const noexcept { return (flags_ & KeyDownMask) != 0; }
    ...
    [[nodiscard]] bool isKeyPressed() const noexcept { return (flags_ & KeyPressedMask) != 0; }
    ...
    [[nodiscard]] bool isKeyReleased() const noexcept { return (flags_ & KeyReleasedMask) != 0; }
    ...
    [[nodiscard]] bool isKeyRepeated() const noexcept { return (flags_ & KeyRepeatedMask) != 0; }
    ...
    [[nodiscard]] bool isMouseDown() const noexcept { return (flags_ & MouseDownMask) != 0; }
    ...
    [[nodiscard]] bool isMouseMove() const noexcept { return (flags_ & MouseMoveMask) != 0; }
    ...
    [[nodiscard]] bool isMousePressed() const noexcept { return (flags_ & MousePressedMask) != 0; }
    ...
    [[nodiscard]] bool isMouseReleased() const noexcept { return (flags_ & MouseReleasedMask) != 0; }
    ...
    [[nodiscard]] bool isMouseScroll() const noexcept { return (flags_ & MouseScrollMask) != 0; }
  ...
  static std::optional<InputKey> GetKeyFromStr(const std::string& key) noexcept {
  ...
  static std::optional<MouseButton> GetMouseButtonFromStr(const std::string& button) noexcept {
    static std::unordered_map<std::string, MouseButton> map{
        ...
        {"1", MOUSE_BUTTON_1},       {"2", MOUSE_BUTTON_2},         {"3", MOUSE_BUTTON_3},
        {"4", MOUSE_BUTTON_4},       {"5", MOUSE_BUTTON_5},         {"6", MOUSE_BUTTON_6},
        {"7", MOUSE_BUTTON_7},       {"8", MOUSE_BUTTON_8},
    };
  ...
  static std::optional<std::string> GetMouseButtonStr(MouseButton button) noexcept {
    static std::unordered_map<MouseButton, std::string> map{
        ...
        {MOUSE_BUTTON_1, "1"},       {MOUSE_BUTTON_2, "2"},         {MOUSE_BUTTON_3, "3"},
        {MOUSE_BUTTON_4, "4"},       {MOUSE_BUTTON_5, "5"},         {MOUSE_BUTTON_6, "6"},
        {MOUSE_BUTTON_7, "7"},       {MOUSE_BUTTON_8, "8"},
    };
    if (auto itr = map.find(button); itr != map.end()) {
      return itr->second;
    }
  ...
  [[nodiscard]] bool anyKeyPressed() const noexcept { return flags_.isKeyPressed(); }

  [[nodiscard]] bool anyKeyReleased() const noexcept { return flags_.isKeyReleased(); }

  [[nodiscard]] bool anyKeyRepeated() const noexcept { return flags_.isKeyRepeated(); }

  [[nodiscard]] bool anyMouseDown() const noexcept { return flags_.isMouseDown(); }

  [[nodiscard]] bool anyMousePressed() const noexcept { return flags_.isMousePressed(); }

  [[nodiscard]] bool anyMouseReleased() const noexcept { return flags_.isMouseReleased(); }

  [[nodiscard]] bool ctrlDown() const noexcept { return (mods_ & MOD_CONTROL) != 0; }

  [[nodiscard]] bool mouseMoved() const noexcept { return flags_.isMouseMove(); }

  [[nodiscard]] bool mouseScrolled() const noexcept { return flags_.isMouseScroll(); }

  [[nodiscard]] bool shiftDown() const noexcept { return (mods_ & MOD_SHIFT) != 0; }

  [[nodiscard]] bool superDown() const noexcept { return (mods_ & MOD_SUPER) != 0; }
  ...
  [[nodiscard]] glm::vec3 cursorWorldPoint() const noexcept { return cursorWorldPoint_; }
  ...
  [[nodiscard]] const KeyStorage& keysDown() const noexcept { return keysDown_; }

  [[nodiscard]] const KeyStorage& keysPressed() const noexcept { return keysPressed_; }

  [[nodiscard]] const KeyStorage& keysReleased() const noexcept { return keysReleased_; }

  [[nodiscard]] const KeyStorage& keysRepeated() const noexcept { return keysRepeated_; }

  [[nodiscard]] const MouseStorage& mouseDown() const noexcept { return mouseDown_; }

  [[nodiscard]] const MouseStorage& mousePressed() const noexcept { return mousePressed_; }

  [[nodiscard]] const MouseStorage& mouseReleased() const noexcept { return mouseReleased_; }

  [[nodiscard]] bool isKeyDown(InputKey key, InputMod mask = 0) const noexcept {
    return anyKeyDown() && (mods_ & mask) == mask && containsVal(&keysDown_, key);
  }

  [[nodiscard]] bool isKeyPressed(InputKey key, InputMod mask = 0) const noexcept {
    return anyKeyPressed() && (mods_ & mask) == mask && containsVal(&keysPressed_, key);
  }

  [[nodiscard]] bool isKeyReleased(InputKey key, InputMod mask = 0) const noexcept {
    return anyKeyReleased() && (mods_ & mask) == mask && containsVal(&keysReleased_, key);
  }

  [[nodiscard]] bool isKeyRepeated(InputKey key, InputMod mask = 0) const noexcept {
    return anyKeyRepeated() && (mods_ & mask) == mask && containsVal(&keysRepeated_, key);
  }

  [[nodiscard]] bool isMouseDown(MouseButton button, InputMod mask = 0) const noexcept {
    return anyMouseDown() && (mods_ & mask) == mask && containsVal(&mouseDown_, button);
  }

  [[nodiscard]] bool isMousePressed(MouseButton button, InputMod mask = 0) const noexcept {
    return anyMousePressed() && (mods_ & mask) == mask && containsVal(&mousePressed_, button);
  }

  [[nodiscard]] bool isMouseReleased(MouseButton button, InputMod mask = 0) const noexcept {
    return anyMouseReleased() && (mods_ & mask) == mask && containsVal(&mouseReleased_, button);
  }
 ...
 private:
  ...
  static bool containsVal(const KeyStorage* arr, InputKey val) noexcept {
    const auto* const itr =
        std::ranges::find_if(arr->begin(), arr->end(), [val](auto v) { return v == _::InputUnknown || v == val; });
