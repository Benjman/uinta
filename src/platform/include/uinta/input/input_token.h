...
constexpr Key extractKey(u32 token) noexcept {
  constexpr auto mask = 0x0000FFFF;
  return static_cast<Key>(token & mask);
}

constexpr MouseBtn extractMouseButton(u32 token) noexcept {
  constexpr auto mask = 0x0000FFFF;
  return static_cast<MouseBtn>(token & mask);
}

constexpr Action extractAction(u32 token) noexcept {
  constexpr auto mask = 0x000F0000;
  return static_cast<Action>(token & mask);
}

constexpr Mod extractMods(u32 token) noexcept {
  constexpr auto mask = 0x0FF00000;
  return static_cast<Mod>(token & mask);
}
