...
std::optional<std::filesystem::path> GetDir() {
  ...
  if constexpr (T == DirType::CacheDir) {
    static const std::filesystem::path dir = std::filesystem::path(_::GetHome()) / "Library" / "Caches";
    return dir;
  } else if constexpr (T == DirType::ConfigDir) {
    static const std::filesystem::path dir = std::filesystem::path(_::GetHome()) / "Library" / "Preferences";
    return dir;
  } else if constexpr (T == DirType::DataDir) {
    static const std::filesystem::path dir = std::filesystem::path(_::GetHome()) / "Library" / "Application Support";
    return dir;
...
#elif defined(_WIN32)
  if constexpr (T == DirType::CacheDir) {
    static const std::filesystem::path dir = _::GetKnownFolder(FOLDERID_LocalAppData);
    return dir;
  } else if constexpr (T == DirType::ConfigDir) {
    static const std::filesystem::path dir = _::GetKnownFolder(FOLDERID_RoamingAppData);
    return dir;
  } else if constexpr (T == DirType::DataDir) {
    static const std::filesystem::path dir = _::GetKnownFolder(FOLDERID_LocalAppData);
    return dir;
