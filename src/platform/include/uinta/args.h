...
struct ArgHandler {
  ...
  [[nodiscard]] virtual constexpr std::span<const std::string> getKeys()
      const noexcept = 0;

  [[nodiscard]] virtual constexpr std::string getPrimaryKey()
      const noexcept = 0;

  virtual Status handle(const std::string& value) noexcept {
  ...
  static Status setValue(ArgHandler* handler, std::string value) noexcept {
    ...
    if (handler->processor_->store_.contains(handler->getArg())) {
      status = AlreadyExistsError(
          absl::StrFormat("%s already exists", handler->getPrimaryKey()));
    }
