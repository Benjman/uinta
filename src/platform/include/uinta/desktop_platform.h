...
class DesktopPlatform : public Platform {
 public:
  explicit DesktopPlatform(AppConfig* config = nullptr, DesktopPlatformApi* = GlfwPlatformApi::Instance()) noexcept;
  ...
  Status swapBuffers() const noexcept override { return api_->swapBuffers(window_.get()); }

  StatusOr<void*> createWindow(Window* window) noexcept { return api_->createWindow(window); }

  Status destroy(const Window* window) const noexcept { return api_->destroy(window); }

  Status setWindowPosition(void* window, u32 x, u32 y) const noexcept { return api_->setWindowPosition(window, x, y); }

  void makeFullscreen(bool makeFullscreen, Window* window, const Monitor* monitor = nullptr) noexcept override;
