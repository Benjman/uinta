...
struct MockPlatform : Platform {
  ...
  ~MockPlatform() noexcept {
    // Destroy window before appConfig is destroyed, since Window's destructor
    // uses appConfig
    window_.reset();
  }

  std::function<void(i32*, i32*)> onGetAndUpdateWindowSize = [](i32*, i32*) -> void {};
  void getAndUpdateWindowSize(i32* w, i32* h) const noexcept override { onGetAndUpdateWindowSize(w, h); }
  ...
  std::function<StatusOr<time_t>()> onRuntime = [this] { return runtimeGetter_.runtime(); };
  ...
  std::function<StatusOr<void*>(const Window*)> onCreateWindow = [](const Window*) -> StatusOr<void*> {
    return nullptr;
  };
  StatusOr<void*> createWindow(const Window* window) const noexcept { return onCreateWindow(window); }

  std::function<Status(const Window*)> onDestroy = [](const Window*) -> Status { return OkStatus(); };
  Status destroy(const Window* window) const noexcept { return onDestroy(window); }
  ...
  void makeFullscreen(bool makeFullscreen, Window* window, const Monitor* monitor) noexcept override {
