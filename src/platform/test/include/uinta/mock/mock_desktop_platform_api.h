...
struct MockDesktopPlatformApi : DesktopPlatformApi {
  std::function<StatusOr<void*>(Window*)> onCreateWindow = [](Window*) -> StatusOr<void*> { return nullptr; };
  StatusOr<void*> createWindow(Window* window) noexcept override { return onCreateWindow(window); }

  std::function<Status(const Window*)> onDestroy = [](const Window*) -> Status { return OkStatus(); };
  Status destroy(const Window* window) const noexcept override { return onDestroy(window); }

  std::function<StatusOr<std::vector<Monitor>>()> onFindMonitors = []() {
    return std::vector<Monitor>{{"Test monitor", 1920, 1080, 144, nullptr, true}};
  };
  StatusOr<std::vector<Monitor>> findMonitors() const noexcept override { return onFindMonitors(); }

  std::function<void(const Window*, i32*, i32*)> onGetWindowPos = [](const Window*, i32* x, i32* y) {
    *x = 0;
    *y = 0;
  };
  void getWindowPos(const Window* window, i32* x, i32* y) const noexcept override { onGetWindowPos(window, x, y); }

  std::function<void(const Window*, i32*, i32*)> onGetWindowSize = [](const Window*, i32* w, i32* h) {
    *w = 800;
    *h = 600;
  };
  void getWindowSize(const Window* window, i32* w, i32* h) const noexcept override { onGetWindowSize(window, w, h); }
