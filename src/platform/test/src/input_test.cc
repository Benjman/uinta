...
TEST_F(InputTest, onCursorPos) {
  ...
  MouseEvent event(nullptr, x, y, -1, -1, 0);
  ...
  ASSERT_EQ(Input::MouseMoveMask, input.flags() & Input::MouseMoveMask);
...
TEST_F(InputTest, onKey_press) {
  ...
  for (size_t i = 0; i < _::InputStorageSize; i++) {
    ...
    ASSERT_EQ(Input::KeyPressedMask | Input::KeyDownMask, input.flags());
    event.key++;
  }
}
...
TEST_F(InputTest, onKey_release) {
  ...
  ASSERT_EQ(Input::KeyReleasedMask, input.flags() & Input::KeyReleasedMask);
}
...
TEST_F(InputTest, onKey_repeated) {
  ...
  ASSERT_FALSE(status.ok());
  ASSERT_TRUE(IsResourceExhausted(status));
  ASSERT_EQ(Input::KeyRepeatedMask, input.flags() & Input::KeyRepeatedMask);
}
...
TEST_F(InputTest, onMouseButton_pressed) {
  ...
  ASSERT_NE(0, input.flags() & Input::MousePressedMask);
  ASSERT_NE(0, input.flags() & Input::MouseDownMask);
}
...
TEST_F(InputTest, onMouseButton_released) {
  ...
  ASSERT_NE(0, input.flags() & Input::MouseReleasedMask);
  ASSERT_EQ(0, input.flags() & Input::MouseDownMask);
}
...
TEST_F(InputTest, onScroll) {
  f32 x = 3, y = 6;
  ASSERT_TRUE(input.onScroll({nullptr, x, y, -1, -1, 0}).ok());
  ASSERT_TRUE(input.mouseScrolled());
  ASSERT_NE(0, input.flags() & Input::MouseScrollMask);
}
...
TEST_F(InputTest, reset_resetKeyboard_keyPressed) {
  ...
  ASSERT_NE(0, input.flags() & Input::KeyPressedMask);
  ASSERT_EQ(event.key, input.keysPressed().at(0));
  input.reset();
  ASSERT_FALSE(input.isKeyPressed(event.key));
  ASSERT_EQ(0, input.flags() & Input::KeyPressedMask);
...
TEST_F(InputTest, reset_resetKeyboard_keyReleased) {
  ...
  ASSERT_NE(0, input.flags() & Input::KeyReleasedMask);
  ASSERT_EQ(event.key, input.keysReleased().at(0));
  input.reset();
  ASSERT_FALSE(input.isKeyReleased(event.key));
  ASSERT_EQ(0, input.flags() & Input::KeyReleasedMask);
...
TEST_F(InputTest, reset_resetMouse_buttonPressed) {
  ...
  ASSERT_EQ(0, input.flags() & Input::MousePressedMask);
...
TEST_F(InputTest, reset_resetMouse_buttonReleased) {
  ...
  ASSERT_EQ(0, input.flags() & Input::MouseReleasedMask);
  ASSERT_EQ(_::InputUnknown, input.mouseReleased().at(0));
}

TEST_F(InputTest, reset_resetMouse_cursorDelta) {
  ASSERT_TRUE(input.onCursorPos({nullptr, 3, 6, -1, -1, 0}).ok());
  ASSERT_NE(0, input.flags() & Input::MouseMoveMask);
  ASSERT_EQ(3, input.cursordx());
  ASSERT_EQ(6, input.cursordy());
  input.reset();
  ASSERT_EQ(0, input.flags() & Input::MouseMoveMask);
  ASSERT_EQ(0, input.cursordx());
  ASSERT_EQ(0, input.cursordy());
}

TEST_F(InputTest, reset_resetMouse_scrollDelta) {
  ASSERT_TRUE(input.onScroll({nullptr, 3, 6, -1, -1, 0}).ok());
  ASSERT_NE(0, input.flags() & Input::MouseScrollMask);
  ASSERT_EQ(3, input.scrolldx());
  ASSERT_EQ(6, input.scrolldy());
  input.reset();
  ASSERT_EQ(0, input.flags() & Input::MouseScrollMask);
  ASSERT_EQ(0, input.scrolldx());
  ASSERT_EQ(0, input.scrolldy());
}
