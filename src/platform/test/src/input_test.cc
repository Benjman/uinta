...
TEST_F(InputTest, GetModsStr_noMods) {
  auto result = Input::GetModsStr(0);
  ASSERT_FALSE(result.has_value());
}

TEST_F(InputTest, GetModsStr_singleMod) {
  ASSERT_EQ("SHIFT", Input::GetModsStr(MOD_SHIFT).value());
  ASSERT_EQ("CTRL", Input::GetModsStr(MOD_CONTROL).value());
  ASSERT_EQ("ALT", Input::GetModsStr(MOD_ALT).value());
  ASSERT_EQ("SUPER", Input::GetModsStr(MOD_SUPER).value());
  ASSERT_EQ("CAPS_LOCK", Input::GetModsStr(MOD_CAPS_LOCK).value());
  ASSERT_EQ("NUM_LOCK", Input::GetModsStr(MOD_NUM_LOCK).value());
}

TEST_F(InputTest, GetModsStr_multipleMods) {
  ASSERT_EQ("SHIFT+CTRL", Input::GetModsStr(MOD_SHIFT | MOD_CONTROL).value());
  ASSERT_EQ("SHIFT+ALT", Input::GetModsStr(MOD_SHIFT | MOD_ALT).value());
  ASSERT_EQ("CTRL+ALT", Input::GetModsStr(MOD_CONTROL | MOD_ALT).value());
  ASSERT_EQ("SHIFT+CTRL+ALT", Input::GetModsStr(MOD_SHIFT | MOD_CONTROL | MOD_ALT).value());
  ASSERT_EQ("SHIFT+CTRL+ALT+SUPER", Input::GetModsStr(MOD_SHIFT | MOD_CONTROL | MOD_ALT | MOD_SUPER).value());
}
