...
class FileTestF : public ::testing::Test {
 protected:
  ...
  static void WriteTestFileBinary(const std::filesystem::path& path, const std::vector<u8>& content) {
    std::ofstream file(path, std::ios::out | std::ios::binary);
...
TEST_F(FileTestF, CopyFileReturnsErrorForNonExistentSource) {
  auto status = CopyFile("/nonexistent/source.txt", testDir_ / "destination.txt");
