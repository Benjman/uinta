...
#include "./args/locale_handler.h"
#include "./args/plugin_path_handler.h"

namespace uinta {

ArgsProcessor::ArgsProcessor(i32 argc, const char** argv) noexcept {
  using KeyValuePair = std::pair<std::string, std::string>;
  constexpr const auto kvp = [&](const std::string& arg) -> KeyValuePair {
  ...
  handlers.push_back(std::make_unique<LocaleHandler>(this));
  handlers.push_back(std::make_unique<PluginPathHandler>(this));
...
std::optional<std::string> ArgsProcessor::getValue(Arg arg) const noexcept {
...
std::optional<bool> ArgsProcessor::getBoolean(Arg arg) const noexcept {
  auto itr = store_.find(arg);
  if (itr == store_.end()) {
    return std::nullopt;
  }
  if (itr->second != ArgsProcessor::True && itr->second != ArgsProcessor::False) {
    return std::nullopt;
  }
