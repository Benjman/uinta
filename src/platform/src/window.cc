...
Window::Window(const Platform* platform, AppConfig* appConfig) noexcept
    : monitor_(platform->primaryMonitor().value_or(nullptr)), appConfig_(appConfig) {
  if (monitor_ == nullptr) {
  ...
  if (auto position = appConfig->getInt2("platform.window.position", buf.data()); position.has_value()) {
    x_ = position.value()[0];
    y_ = position.value()[1];
  } else {
  ...
  if (auto size = appConfig->getInt2("platform.window.size", buf.data()); size.has_value()) {
    width_ = size.value()[0];
  ...
  LOG(INFO) << absl::StrFormat("Window: name='%s', size=%dx%d, position=(%d,%d), fullscreen=%s", name_, width_, height_,
                               x_, y_, isFullscreen_ ? "yes" : "no");
}
...
Window::~Window() noexcept {
  ...
  if (auto status = appConfig_->setInt2("platform.window.position", buf.data()); !status.ok()) {
    LOG(WARNING) << "Failed to serialize window position: " << status.message();
  }

  buf[0] = static_cast<i32>(width_);
  buf[1] = static_cast<i32>(height_);
  if (auto status = appConfig_->setInt2("platform.window.size", buf.data()); !status.ok()) {
    LOG(WARNING) << "Failed to serialize window size: " << status.message();
  }

  if (auto status = appConfig_->setBoolean("platform.window.fullscreen", isFullscreen_); !status.ok()) {
    LOG(WARNING) << "Failed to serialize window fullscreen state: " << status.message();
  }

  if (auto status = appConfig_->setString("platform.window.name", name_); !status.ok()) {
