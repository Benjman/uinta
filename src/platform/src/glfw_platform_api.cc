...
Status GlfwPlatformApi::setWindowPosition(void* window, u32 x,
                                          u32 y) const noexcept {
  assert(window && "`void* window` cannot be null.");
  glfwSetWindowPos(static_cast<GLFWwindow*>(window), static_cast<i32>(x),
                   static_cast<i32>(y));
  return OkStatus();
}
...
Status GlfwPlatformApi::registerInputHandlers(const Window* window,
  ...
  glfwSetScrollCallback(glfwWindow, [](auto* window, f64 x, f64 y) {
    auto* data = static_cast<WindowData*>(glfwGetWindowUserPointer(window));
    MouseEvent event(static_cast<f32>(x), static_cast<f32>(y), InputUnknown,
                     _::InputUnknown, 0);
