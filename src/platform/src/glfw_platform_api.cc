...
StatusOr<std::vector<Monitor>> GlfwPlatformApi::findMonitors() const noexcept {
  ...
  if (monitorPtrs == nullptr) {
    return InternalError("Call to `glfwGetMonitors()` failed.");
  }
  ...
  for (auto i = 0; i < monitorCount; i++) {
    ...
    const auto* name = glfwGetMonitorName(monitorPtr);
    auto isPrimary = monitorPtr == primaryMonitor;
    auto& monitor = monitors.emplace_back(name, mode->width, mode->height, mode->refreshRate, monitorPtr, isPrimary);
    LOG(INFO) << absl::StrFormat("Found monitor %s %ix%i@%ihz%s", monitor.name(), monitor.width(), monitor.height(),
                                 monitor.hz(), monitor.flags().isPrimary() ? " (Primary)" : "");
  }
  ...
  glfwSetMonitorCallback([](auto* monitorPtr, const auto event) {
    ...
    if (auto* api = GlfwPlatformApi::Instance(); api && api->windowData_.platform) {
      ...
      auto* monitor = savedMonitor.value_or(api->windowData_.platform->addMonitor(
          {name, mode->width, mode->height, mode->refreshRate, monitorPtr, isPrimary, isConnected}));
      api->windowData_.platform->dispatch<PlatformEvent::OnMonitorChange>(OnMonitorChangeEvent(monitor));
    }
  });
...
StatusOr<void*> GlfwPlatformApi::createWindow(Window* window) noexcept {
  assert(window->monitor() && "A monitor is required to create a window.");

  constexpr auto versionMajor = 4;
  constexpr auto versionMinor = 3;
  LOG(INFO) << absl::StrFormat("Initializing GLFW v%i.%i with OpenGL Core profile...", versionMajor, versionMinor);
  ...
  glfwSetErrorCallback([](auto code, const auto* description) {
    ...
    if (api->windowData_.platform) {
      api->windowData_.platform->dispatch<PlatformEvent::OnError>(OnErrorEvent(code, description));
    }
  });
  ...
  i32 width = static_cast<i32>(window->width());
  i32 height = static_cast<i32>(window->height());

  auto* ptr = glfwCreateWindow(width, height, name.data(), monitor, nullptr);
  if (ptr == nullptr) {
  ...
  glfwSetWindowCloseCallback(ptr, [](auto* window) {
    ...
    data->platform->dispatch<PlatformEvent::OnCloseRequest>(OnCloseRequestEvent(window));
  });

  glfwSetFramebufferSizeCallback(ptr, [](auto* window, i32 width, i32 height) {
    auto* data = static_cast<WindowData*>(glfwGetWindowUserPointer(window));
    auto* platform = data->platform;
    assert(platform && "Platform was null when framebuffer size callback was invoked.");
    if (auto* platformWindow = platform->window()) {
      platformWindow->size(width, height);
    }
    platform->dispatch<PlatformEvent::OnViewportSizeChange>(OnViewportSizeChangeEvent(window, width, height));
  });

#ifdef UINTA_DEBUG
#ifdef GL_VERSION_4_3
  if (glfwExtensionSupported("GL_KHR_debug") != 0) {
    glDebugMessageCallback(
        [](auto source, auto type, auto id, auto severity, auto length, const auto* message, const auto* userParam) {
          const auto* data = static_cast<const WindowData*>(userParam);
          auto* platform = data->platform;
          assert(platform && "Platform was null when debug message callback was invoked.");
          platform->dispatch<PlatformEvent::OnDebugMessage>(
              OnDebugMessageEvent(source, type, id, severity, length, message));
        },
        &windowData_);
...
void GlfwPlatformApi::getWindowPos(const Window* window, i32* x, i32* y) const noexcept {
  if (window != nullptr) {
    glfwGetWindowPos(static_cast<GLFWwindow*>(window->userData()), x, y);
  }
}

void GlfwPlatformApi::getWindowSize(const Window* window, i32* w, i32* h) const noexcept {
  if (window != nullptr) {
...
StatusOr<time_t> GlfwPlatformApi::runtime() const noexcept { return glfwGetTime(); }

Status GlfwPlatformApi::setWindowPosition(void* window, u32 x, u32 y) const noexcept {
  assert(window && "`void* window` cannot be null.");
  glfwSetWindowPos(static_cast<GLFWwindow*>(window), static_cast<i32>(x), static_cast<i32>(y));
  return OkStatus();
}
...
Status GlfwPlatformApi::destroy(const Window* window) const noexcept {
  if ((window != nullptr) && (window->userData() != nullptr)) {
    try {
...
Status GlfwPlatformApi::registerInputHandlers(const Window* window, Input* input) noexcept {
  assert(input && "Input* cannot be null.");
  if (windowData_.input != nullptr) {
    return CancelledError("Unexpected call to `GlfwPlatformApi::registerInputHandlers()`.");
  }
  ...
  glfwSetKeyCallback(glfwWindow, [](auto* window, i32 key, i32 scancode, i32 action, i32 mods) {
    auto* data = static_cast<WindowData*>(glfwGetWindowUserPointer(window));
  ...
  glfwSetCursorPosCallback(glfwWindow, [](auto* window, f64 x, f64 y) {
    auto* data = static_cast<WindowData*>(glfwGetWindowUserPointer(window));
    MouseEvent event(static_cast<f32>(x), static_cast<f32>(y), -1, -1, 0);
  ...
  glfwSetScrollCallback(glfwWindow, [](auto* window, f64 x, f64 y) {
    auto* data = static_cast<WindowData*>(glfwGetWindowUserPointer(window));
    MouseEvent event(static_cast<f32>(x), static_cast<f32>(y), InputUnknown, _::InputUnknown, 0);
    if (auto status = data->input->onScroll(event); !status.ok()) {
      LOG(WARNING) << status.message();
    }
  });

  glfwSetMouseButtonCallback(glfwWindow, [](auto* window, i32 button, i32 action, i32 mods) {
    auto* data = static_cast<WindowData*>(glfwGetWindowUserPointer(window));
