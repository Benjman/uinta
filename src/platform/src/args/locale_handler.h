...
struct LocaleHandler : ArgHandler {
  ...
  explicit LocaleHandler(ArgsProcessor* processor) noexcept : ArgHandler(processor) {}

  [[nodiscard]] ArgsProcessor::Arg getArg() const noexcept override { return ArgsProcessor::Locale; }

  [[nodiscard]] constexpr std::span<const std::string> getKeys() const noexcept override { return Keys; }

  [[nodiscard]] constexpr std::string getPrimaryKey() const noexcept override { return PrimaryKey; }
};
