...
struct HelpHandler : ArgHandler {
  static constexpr std::array<std::string, 2> Keys = std::array<std::string, 2>{"-h", "--help"};

  static constexpr std::string PrimaryKey = Keys[1];

  explicit HelpHandler(ArgsProcessor* processor) noexcept : ArgHandler(processor) {}

  [[nodiscard]] ArgsProcessor::Arg getArg() const noexcept override { return ArgsProcessor::Help; }

  [[nodiscard]] constexpr std::span<const std::string> getKeys() const noexcept override { return Keys; }

  [[nodiscard]] std::string getPrimaryKey() const noexcept override { return PrimaryKey; }
