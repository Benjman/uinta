...
Status ReadFile(const std::filesystem::path& path, std::string& out) noexcept {
  ...
  if (!file) {
    return NotFoundError(absl::StrFormat("Failed to open file: %s", path.string()));
  }

  file.seekg(0, std::ios::end);
  auto pos = file.tellg();
  if (pos == -1) {
    return InternalError(absl::StrFormat("Failed to get file size: %s", path.string()));
  }
  ...
  if (!file.read(out.data(), static_cast<std::streamsize>(size))) {
    return InternalError(absl::StrFormat("Failed to read file: %s", path.string()));
  }
...
Status ReadFileLines(const std::filesystem::path& path, std::vector<std::string>& out) noexcept {
  ...
  if (file.bad()) {
    return InternalError(absl::StrFormat("Error reading file: %s", path.string()));
  }
...
Status CopyFile(const std::filesystem::path& src, const std::filesystem::path& dst,
  ...
  if (auto parent = dst.parent_path(); !parent.empty()) {
    if (!std::filesystem::exists(parent, ec)) {
      ...
      if (ec) {
        return InternalError(absl::StrFormat("Failed to create directory %s: %s", parent.string(), ec.message()));
      }
    }
  }
  ...
  if (ec) {
    return InternalError(absl::StrFormat("Failed to copy %s to %s: %s", src.string(), dst.string(), ec.message()));
  }

  return OkStatus();
}

FileReader::FileReader(std::ifstream stream, std::filesystem::path path) noexcept
    : stream_(std::move(stream)), path_(std::move(path)) {}

Status FileReader::Open(const std::filesystem::path& path, FileReader& out) noexcept {
...
Status FileReader::ReadLine(std::optional<std::string>& out) noexcept {
  ...
  return InternalError(absl::StrFormat("Error reading line from: %s", path_.string()));
}
...
Status FileReader::ReadAllLines(std::vector<std::string>& out) noexcept {
  ...
  if (stream_.bad()) {
    return InternalError(absl::StrFormat("Error reading file: %s", path_.string()));
  }
...
Status FileReader::ReadAll(std::string& out) noexcept {
  ...
  if (start == -1 || end == -1) {
    return InternalError(absl::StrFormat("Failed to get file size: %s", path_.string()));
  }
  ...
  if (!stream_.read(out.data(), static_cast<std::streamsize>(size))) {
    return InternalError(absl::StrFormat("Failed to read file: %s", path_.string()));
  }
...
Status FileReader::ReadAllBinary(std::vector<u8>& out) noexcept {
  ...
  out.resize(size);
  if (!stream_.read(reinterpret_cast<char*>(out.data()), static_cast<std::streamsize>(size))) {
    return InternalError(absl::StrFormat("Failed to read file: %s", path_.string()));
  }
