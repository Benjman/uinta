...
void uinta::loadObj(const std::string& objBuffer, float* const vbuf, unsigned int* vcount, unsigned int* const ibuf,
  ...
  parseFile(objBuffer, vertex_lines, uv_lines, normal_lines, face_lines);
  ...
  *icount += face_lines.size() * 3;
  ...
  if (const MeshAttrib* attrib = findMeshAttrib(MeshAttribType_UV, attribs)) packUVs(*attrib, vbuf, vcount, face_data, uvs);
...
**/
void uinta::parseFile(const std::string& objBuffer, std::vector<std::string>& vertex_lines, std::vector<std::string>& uv_lines,
