...
std::map<size_t, size_t> uinta::gl_state::glStates[MAX_GL_STATE_TYPES];

bool uinta::gl_state::isActive(GlStateType type, size_t key, size_t secondary) {
...
bool uinta::gl_state::isActiveElseSet(GlStateType type, size_t key, size_t secondary) {
	if (secondary == INT32_MAX) {
		// Method called without secondary value:
		secondary = key;
		key = 0;
