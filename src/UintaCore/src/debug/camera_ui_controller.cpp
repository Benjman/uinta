...
void CameraUiController::update(const EngineState &state) {
	...
	const std::string &val = "P" + std::to_string(std::fmod(_pitch, 360.f));
...
void YawController::update(const EngineState &state) {
	...
	_yaw = _camera->getYaw();
	const std::string &val = "Y" + std::to_string(std::fmod(_yaw, 360.f));
	setTextValue(val.c_str());
	populateMesh();
	uploadMesh();
}
...
void ZoomController::update(const EngineState &state) {
	...
	const std::string &val = "Z" + std::to_string(std::fmod(_zoom, 360.f));
	setTextValue(val.c_str());
	populateMesh();
	uploadMesh();
}
