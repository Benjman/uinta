...
extern void uinta::generateMesh(const Quad *qt, float *vertexBuffer, unsigned int *indexBuffer, unsigned int *vertexCount,
  ...
  *indexOffset += 8u;
  ...
  if (qt->bottomLeft && qt->bottomLeft->isActive())
    generateMesh(qt->bottomLeft, &vertexBuffer[localVertexCount], &indexBuffer[localIndexCount], &localVertexCount,
                 &localIndexCount, indexOffset, width, height);
  if (qt->bottomRight && qt->bottomRight->isActive())
    generateMesh(qt->bottomRight, &vertexBuffer[localVertexCount], &indexBuffer[localIndexCount], &localVertexCount,
                 &localIndexCount, indexOffset, width, height);
  if (qt->topLeft && qt->topLeft->isActive())
    generateMesh(qt->topLeft, &vertexBuffer[localVertexCount], &indexBuffer[localIndexCount], &localVertexCount, &localIndexCount,
                 indexOffset, width, height);
  if (qt->topRight && qt->topRight->isActive())
    generateMesh(qt->topRight, &vertexBuffer[localVertexCount], &indexBuffer[localIndexCount], &localVertexCount,
                 &localIndexCount, indexOffset, width, height);
