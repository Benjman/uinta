...
Camera::Camera(const CameraConfig &config)
    : config(CameraConfig(config)), controls(this->config), target(config.startTarget),
      angle(config.startAngle, config.rotateAgility), dist(config.startZoom, config.zoomAgility) {}
