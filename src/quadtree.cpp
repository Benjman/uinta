...
void validateQuad(const Quad& quad) {
  ...
  if (ceilf(quad.topLeftBounds.y) != quad.topLeftBounds.y) throw std::runtime_error("quad.topLeftBounds.y must be an integer.");
  ...
  if (width < quad.minCellSize) throw std::runtime_error("quad width must be at least the size of the minimum cell size");
  if (height < quad.minCellSize) throw std::runtime_error("quad height must be at least the size of the minimum cell size");

  if (width - height) throw std::runtime_error("quad boundaries must be a square");

  if ((width & (width - 1)) != 0 || (height & (height - 1)) != 0) throw std::runtime_error("quad size must be a power of 2");
...
void Quad::addEntity(entt::entity entity) noexcept {
  if (entityCount + 1 > entityStoreSize) {
    ...
    if (entityCount > 0) memmove(arr, entityStore, sizeof(entt::entity) * entityCount);
...
void Quad::clear() noexcept {
  for (auto i = 0; i < entityCount; i++) removeEntity(entityStore[i]);

  if (bottomLeft != nullptr) bottomLeft->clear();
  if (bottomRight != nullptr) bottomRight->clear();
