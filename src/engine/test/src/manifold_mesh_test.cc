...
TEST(ManifoldMeshTest, CubeHasTriangulatedFaces) {
  ...
  for (const auto& v : mesh.vertices()) {
    ASSERT_NEAR(0.5f, std::max({std::abs(v.position.x), std::abs(v.position.y), std::abs(v.position.z)}), kEpsilon);
  }
}
...
TEST(ManifoldMeshTest, CubeNormalsAreAxisAlignedAtDefaultSharpness) {
  ...
  for (const auto& v : mesh.vertices()) {
    uniqueNormals.emplace(static_cast<int>(std::round(v.normal.x)), static_cast<int>(std::round(v.normal.y)),
