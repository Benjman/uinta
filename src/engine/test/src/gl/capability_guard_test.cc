...
struct TestCapabilityGuard final : CapabilityGuard {
  TestCapabilityGuard(GLenum capability, bool isActive, const OpenGLApi* gl) noexcept
...
TEST_F(CapabilityGuardTest, StateReversionWhenDisabled) {
  {
    ...
    ASSERT_EQ(invalidCap, disableCalledCap) << "glDisable() shouldn't have been called until destructor.";
  }
...
TEST_F(CapabilityGuardTest, WasEnabledNoChange) {
...

  {
    TestCapabilityGuard guard(cap, true, &gl);
    ASSERT_EQ(cap, isEnabledCalledCap) << "glIsEnabled() wasn't called.";
    ASSERT_EQ(invalidCap, enableCalledCap) << "glEnable() shouldn't be called when already enabled.";
  }
  ASSERT_EQ(invalidCap, disableCalledCap) << "glDisable() shouldn't be called when capability was already enabled.";
}
...
TEST_F(CapabilityGuardTest, InactiveGuardNoEnable) {
  {
    ...
    ASSERT_EQ(cap, isEnabledCalledCap) << "glIsEnabled() wasn't called.";
    ASSERT_EQ(invalidCap, enableCalledCap) << "glEnable() shouldn't be called when isActive=false.";
  }
}
...
TEST_F(CapabilityGuardTest, InactiveGuardNoDisableOnDestruct) {
  ...
  ASSERT_EQ(invalidCap, disableCalledCap) << "glDisable() shouldn't be called for inactive guard.";
}

TEST_F(CapabilityGuardTest, ManualActivate) {
  TestCapabilityGuard guard(cap, false, &gl);
  ASSERT_EQ(invalidCap, enableCalledCap) << "glEnable() shouldn't be called before activate().";

  guard.activate();
  ASSERT_EQ(cap, enableCalledCap) << "glEnable() wasn't called after activate().";
...
TEST_F(CapabilityGuardTest, ManualActivateTwiceOnlyEnablesOnce) {
  ...
  ASSERT_EQ(1, enableCallCount) << "glEnable() should only be called once even with multiple activate() calls.";
}
...
TEST_F(CapabilityGuardTest, ManualActivateDisablesOnDestruct) {
  ...
  ASSERT_EQ(cap, disableCalledCap) << "glDisable() should be called on destruction after manual activate().";
}
