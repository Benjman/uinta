...
TEST_F(FboTest, Constructor) {
  ...
  GLuint expId = 42;
  GLuint expCount = 1;
  gl.onGenFramebuffers = [&expId, &expCount, &onGenFramebuffersCalled](auto count, auto* id) {
    *id = expId;
...
TEST_F(FboTest, Destructor) {
  ...
  gl.onDeleteFramebuffers = [expCount, expId, &onDeleteFramebuffersCalled](auto count, auto* id) {
    ASSERT_EQ(expCount, count);
    ASSERT_EQ(expId, *id);
...
TEST_F(FboTest, AttachColorRenderbuffer) {
  ...
  gl.onGenFramebuffers = [&fboId](auto, auto* id) { *id = fboId; };
  gl.onFramebufferRenderbuffer = [&](auto, auto attachment, auto, auto rbId) {
