...
TEST_F(EngineTest, ConstructorOnClosingListening) {
  ...
  platform.dispatch<PlatformEvent::OnCloseRequest>(OnCloseRequestEvent(nullptr));
...
TEST_F(EngineTest, ConstructorOnViewportSizeChangeListening) {
  ...
  gl.onViewport = [&onViewportCalled](auto, auto, auto, auto) { onViewportCalled = true; };
  platform.dispatch<PlatformEvent::OnViewportSizeChange>(OnViewportSizeChangeEvent(nullptr, 0, 0));
...
TEST_F(EngineTest, ConstructorClearColorSet) {
  ...
  gl.onClearColor = [&](GLfloat, GLfloat, GLfloat, GLfloat) { clearColorCalled = true; };
...
TEST_F(EngineTest, StatusNotOkOnRun) {
  ...
  engine.dispatchers()->addListener<EngineEvent::TickComplete>(
      [&](const auto&) { ASSERT_TRUE(false) << "Entering the run loop was not expected."; });
}

TEST_F(EngineTest, SingleTickAdvancement) {
  MockPlatform platform;
  auto engine = makeEngine(&platform);
  engine.dispatchers()->addListener<EngineEvent::TickComplete>([&](const auto&) { engine.state().isClosing(true); });
  ASSERT_EQ(0, engine.state().tick());
  engine.run();
...
TEST_F(EngineTest, SwapBuffersCalled) {
  ...
  engine.dispatchers()->addListener<EngineEvent::RenderComplete>([&](const auto&) { engine.state().isClosing(true); });
  engine.run();
...
TEST_F(EngineTest, EventTickComplete) {
  ...
  engine.dispatchers()->addListener<EngineEvent::TickComplete>([&](const auto&) {
...
TEST_F(EngineTest, EventRenderComplete) {
  ...
  engine.dispatchers()->addListener<EngineEvent::RenderComplete>([&](const auto&) {
...
TEST_F(EngineTest, EventOnMonitorChange) {
  ...
  auto firstMonExpectedAdvance = 1.0 / static_cast<time_t>(primaryMonitor.value()->hz());
  ASSERT_FLOAT_EQ(firstMonExpectedAdvance, engine.frameManager().frequency);

  Monitor secondMon("Second monitor", 1920, 1080, 144);
  platform.dispatch<PlatformEvent::OnMonitorChange>(OnMonitorChangeEvent(&secondMon));
...
TEST_F(EngineTest, PerStageDeltaTimeCalculation) {
  ...
  class TestScene : public Scene {
   public:
    ...
    TestScene(Engine* engine, time_t* preTick, time_t* tick, time_t* postTick, time_t* preRender, time_t* render,
              time_t* postRender)
        : Scene(engine),
    ...
    void postRender(time_t delta) noexcept override { *postRenderDelta_ = delta; }
  };

  engine.addScene<TestScene>(&preTickDelta, &tickDelta, &postTickDelta, &preRenderDelta, &renderDelta,
                             &postRenderDelta);
...
TEST_F(EngineTest, DifferentDeltasPerStage) {
  ...
  class TestScene : public Scene {
   public:
    ...
    TestScene(Engine* engine, std::vector<time_t>* preTick, std::vector<time_t>* tick, std::vector<time_t>* render)
        : Scene(engine), preTickDeltas_(preTick), tickDeltas_(tick), renderDeltas_(render) {}

    void preTick(time_t delta) noexcept override { preTickDeltas_->push_back(delta); }
    void tick(time_t delta) noexcept override { tickDeltas_->push_back(delta); }
    void render(time_t delta) noexcept override { renderDeltas_->push_back(delta); }
  };
...
TEST_F(EngineTest, IndependentStageDeltaTimes) {
  ...
  class TestScene : public Scene {
   public:
    ...
    TestScene(Engine* engine, std::vector<StageCall>* calls) : Scene(engine), stageCalls_(calls) {}

    void preTick(time_t delta) noexcept override { stageCalls_->push_back({.stage = "preTick", .delta = delta}); }
    void tick(time_t delta) noexcept override { stageCalls_->push_back({.stage = "tick", .delta = delta}); }
    void postTick(time_t delta) noexcept override { stageCalls_->push_back({.stage = "postTick", .delta = delta}); }
    void preRender(time_t delta) noexcept override { stageCalls_->push_back({.stage = "preRender", .delta = delta}); }
    void render(time_t delta) noexcept override { stageCalls_->push_back({.stage = "render", .delta = delta}); }
    void postRender(time_t delta) noexcept override { stageCalls_->push_back({.stage = "postRender", .delta = delta}); }
  };
  ...
  EXPECT_GT(stageCalls.size(), 6) << "Expected at least 2 frames worth of stage calls";
