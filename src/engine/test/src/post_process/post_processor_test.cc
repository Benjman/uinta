...
class RecordingPass : public PostProcessPass {
 public:
  ...
  RecordingPass(const OpenGLApi* /*gl*/, std::vector<Record>* log) noexcept : log_(log) {}

  void execute(const PassContext& ctx) noexcept override { log_->push_back({ctx.input, ctx.output}); }
...
struct PostProcessorTest : public UintaTestF {
 protected:
  void SetUp() override {
    UintaTestF::SetUp();
    gl.onCheckFramebufferStatus = [](auto) -> GLenum { return GL_FRAMEBUFFER_COMPLETE; };
...
TEST_F(PostProcessorTest, BeginSimulationBindsSceneFbo) {
  ...
  gl.onBindFramebuffer = [&binds](GLenum target, GLuint id) { binds.emplace_back(target, id); };
...
TEST_F(PostProcessorTest, ViewportSizeChangeTriggersResize) {
  ...
  auto engine = makeEngine(&platform);
  PostProcessor pp(&engine);
  engine.dispatchers()->dispatch<EngineEvent::ViewportSizeChange>(ViewportSizeChange{800, 600});
...
TEST_F(PostProcessorTest, RenderLayerChangeToUiRunsChain) {
  ...
  engine.dispatchers()->dispatch<EngineEvent::RenderLayerChange>(RenderLayerChange{SceneLayer::UI});
...
TEST_F(PostProcessorTest, RenderLayerChangeToSimulationDoesNotRunChain) {
  ...
  engine.dispatchers()->dispatch<EngineEvent::RenderLayerChange>(RenderLayerChange{SceneLayer::Simulation});
