...
bool Vec3NearEqual(const glm::vec3& a, const glm::vec3& b, float epsilon = kEpsilon) {
...
TEST_F(MeshTest, TranslateMovesAllPositions) {
  ...
  ASSERT_TRUE(Vec3NearEqual(glm::vec3(10, 20, 30), mesh.vertices()[0].position));
  ASSERT_TRUE(Vec3NearEqual(glm::vec3(11, 20, 30), mesh.vertices()[1].position));
}
