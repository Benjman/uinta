...
TEST(RingBuffer, EmptyBufferIncrementFailure) {
  ...
  std::for_each(begin, end, [&](auto) { FAIL() << "Iterator shouldn't iterate on an empty buffer."; });
}
...
TEST(RingBuffer, SingleElement) {
  ...
  std::vector<i32> vec;
  vec.push_back(1);
  std::ranges::for_each(vec.begin(), vec.end(), [&](auto) { iteratorCalled = true; });
  RingBuffer<i32, 1> buffer;
  buffer.push(2);
  auto begin = buffer.begin();
  auto end = buffer.end();
...
TEST(RingBuffer, MultipleElementOverflow) {
  ...
  ASSERT_EQ(elements.at(0), *std::next(buffer.begin(), 0)) << "First element should be " << elements.at(0);

  buffer.push(elements.at(1));
  ASSERT_EQ(2, buffer.size()) << "Buffer size should be 2 after second push.";
  ASSERT_EQ(elements.at(0), *std::next(buffer.begin(), 0)) << "First element should still be " << elements.at(0);
  ASSERT_EQ(elements.at(1), *std::next(buffer.begin(), 1)) << "Second element should be " << elements.at(1);

  buffer.push(elements.at(2));
  ASSERT_EQ(3, buffer.size()) << "Buffer size should be 3 after third push.";
  ASSERT_EQ(elements.at(0), *std::next(buffer.begin(), 0)) << "First element should still be " << elements.at(0);
  ASSERT_EQ(elements.at(1), *std::next(buffer.begin(), 1)) << "Second element should still be " << elements.at(1);
  ASSERT_EQ(elements.at(2), *std::next(buffer.begin(), 2)) << "Third element should be " << elements.at(2);
  ASSERT_EQ(elements.at(0) + elements.at(1) + elements.at(2), std::accumulate(buffer.begin(), buffer.end(), 0))
      << "Sum of buffer elements should be " << (elements.at(0) + elements.at(1) + elements.at(2));

  buffer.push(elements.at(3));
  ASSERT_EQ(3, buffer.size()) << "Buffer size should still be 3 after overflow push.";
  ...
  ASSERT_EQ(elements.at(2), *std::next(buffer.begin(), 1))
      << "Second element should now be " << elements.at(2) << " after overflow.";
  ...
  ASSERT_EQ(elements.at(1) + elements.at(2) + elements.at(3), std::accumulate(buffer.begin(), buffer.end(), 0))
      << "Sum of buffer elements after overflow should be " << (elements.at(1) + elements.at(2) + elements.at(3));
