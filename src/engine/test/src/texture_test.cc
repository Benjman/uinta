...
TEST_F(TextureTest, ConstructorBindsTextureOnCreation) {
  ...
  gl.onBindTexture = [&](GLenum target, GLuint id) { bindCalls.emplace_back(target, id); };
...
TEST_F(TextureTest, ConstructorWithDimensionsCallsTexImage2D) {
  ...
  gl.onTexImage2D = [&](GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border,
...
TEST_F(TextureTest, ConstructorWithZeroDimensionsSkipsTexImage2D) {
  ...
  gl.onTexImage2D = [&](GLenum, GLint, GLint, GLsizei, GLsizei, GLint, GLenum, GLenum, const void*) {
    texImage2DCalled = true;
  };
...
TEST_F(TextureTest, ConstructorWithZeroFilterSkipsFiltering) {
  ...
  gl.onTexParameteri = [&](GLenum, GLenum pname, GLint) { texParamPnames.push_back(pname); };
...
TEST_F(ActiveTextureGuardTest, DestructorRestoresPreviousActiveTexture) {
  ...
  gl.onActiveTexture = [&](GLenum texture) { activeTextureCalls.push_back(texture); };
...
TEST_F(TextureGuardTest, UsesTextureTarget) {
  ...
  gl.onBindTexture = [&](GLenum target, GLuint) { boundTargets.push_back(target); };
