...
TEST_F(VboTest, Constructor) {
  ...
  GLuint expId = 11;
  GLuint expCount = 1;
  gl.onGenBuffers = [&expId, &expCount, &onGenBuffersCalled](auto count, auto* id) {
    *id = expId;
...
TEST_F(VboTest, Destructor) {
  ...
  gl.onDeleteBuffers = [expCount, expId, &onDeleteBuffersCalled](auto count, auto* id) {
    ASSERT_EQ(expCount, count);
    ASSERT_EQ(expId, *id);
...
TEST_F(VboTest, ConstructorWithSize) {
  ...
  gl.onBufferData = [&expTarget, &expSize, &expData, &expUsage, &onBufferDataCalled](auto target, auto size,
...
TEST_F(VboTest, ResizeWithExistingBuffer) {
  ...
  gl.onBindBuffer = [&](auto target, auto id) { bindCalls.push_back({target, id}); };

  bool copyBufferSubDataCalled = false;
  gl.onCopyBufferSubData = [&](auto readTarget, auto writeTarget, auto readOffset, auto writeOffset, auto size) {
...
TEST_F(VboTest, BufferDataWithBufferSegmentOffset) {
  ...
  BufferSegment segment1 = vbo.bufferData(data1.data(), data1.size() * sizeof(f32), GL_STATIC_DRAW);
  ...
  BufferSegment segment2 = vbo.bufferData(data2.data(), data2.size() * sizeof(f32), GL_STATIC_DRAW, segment1);
...
TEST_F(VboTest, ResizeSameSizeNoOp) {
  ...
  gl.onBufferData = [&](auto, auto, const auto*, auto) { bufferDataCallCount++; };
