...
TEST(ComponentManager, InitialState) {
  ...
  ASSERT_EQ(0, componentManager.newFrameComponents().size());
...
TEST(ComponentManager, AddNewFrameComponent) {
  ComponentManager componentManager(&runtimeGetter);
  testAddComponent<NewFrameComponent>(&componentManager,
                                      componentManager.newFrameComponents());
}
...
TEST(ComponentManager, RemoveNewFrameComponent) {
  ComponentManager componentManager(&runtimeGetter);
  testRemoveComponent<NewFrameComponent>(&componentManager,
                                         componentManager.newFrameComponents());
}
...
TEST(ComponentManager, FindNewFrameComponent) {
  ComponentManager componentManager(&runtimeGetter);
  auto* component = componentManager.add<MockComponent<NewFrameComponent>>();
  ASSERT_EQ(component,
            componentManager.find<MockComponent<NewFrameComponent>>());
}
