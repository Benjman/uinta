...
TEST_F(ServiceEventTest, ServiceRegisteredEventFired) {
  ...
  engine.dispatchers()->addListener<EngineEvent::ServiceRegistered>([&](const ServiceRegistered& event) {
    eventFired = true;
...
TEST_F(ServiceEventTest, ServiceUnregisteredEventFired) {
  ...
  engine.dispatchers()->addListener<EngineEvent::ServiceUnregistered>([&](const ServiceUnregistered& event) {
    eventFired = true;
    receivedType = event.type;
  });
