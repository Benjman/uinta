...
class InputSystemTest : public ::testing::Test {
 protected:
  ...
  void simulateMouseMove(f32 x, f32 y) {
    ...
    auto _ = input_.onCursorPos(event);
  }

  // Helper to simulate mouse scroll
  void simulateMouseScroll(f32 dx, f32 dy) {
    MouseEvent event{dx, dy, _::InputUnknown, _::InputUnknown, 0};
    auto _ = input_.onScroll(event);
  }

  Input input_;
  InputSystem system_ = InputSystem(&input_);
};
...
TEST_F(InputSystemTest, MouseMove) {
  int callCount = 0;
  f32 receivedX = 0, receivedY = 0, receivedDx = 0, receivedDy = 0;
