...
TEST_F(InputSystemTest, SubscribeKeySimplePress) {
  ...
  auto _ = system_->subscribeKey(Key::W | Action::Press, [&](Key key, Action action, Mod /*mods*/) {
    callCount++;
    receivedKey = key;
...
TEST_F(InputSystemTest, SubscribeKeyWithModifier) {
  ...
  auto _ = system_->subscribeKey(Key::S | Action::Press | Mod::Shift, [&](Key /*key*/, Action /*action*/, Mod mods) {
    callCount++;
    receivedMods = mods;
  });
...
TEST_F(InputSystemTest, ExactModifierMatching) {
  ...
  auto _ = system_->subscribeKey(Key::A | Action::Press | Mod::Shift, [&](Key, Action, Mod) { callCount++; });
...
TEST_F(InputSystemTest, MultipleModifiers) {
  ...
  auto _ =
      system_->subscribeKey(Key::Q | Action::Press | Mod::Control | Mod::Shift, [&](Key, Action, Mod) { callCount++; });
...
TEST_F(InputSystemTest, KeyRelease) {
  ...
  auto _ = system_->subscribeKey(Key::Escape | Action::Release, [&](Key, Action action, Mod) {
    callCount++;
    receivedAction = action;
  });
...
TEST_F(InputSystemTest, MultipleSubscriptionsToSameKey) {
  ...
  auto _ = system_->subscribeKey(Key::W | Action::Press, [&](Key, Action, Mod) { count1++; });
  _ = system_->subscribeKey(Key::W | Action::Press, [&](Key, Action, Mod) { count2++; });
...
TEST_F(InputSystemTest, UnsubscribeKey) {
  ...
  auto handle = system_->subscribeKey(Key::W | Action::Press, [&](Key, Action, Mod) { callCount++; });
...
TEST_F(InputSystemTest, SubscribeMouseButton) {
  ...
  auto _ = system_->subscribeMouse(MouseBtn::Left | Action::Press, [&](MouseBtn button, Action, Mod) {
    callCount++;
...
TEST_F(InputSystemTest, MouseButtonWithModifier) {
  ...
  auto _ =
      system_->subscribeMouse(MouseBtn::Right | Action::Press | Mod::Alt, [&](MouseBtn, Action, Mod) { callCount++; });
...
TEST_F(InputSystemTest, UnsubscribeMouseMove) {
  ...
  auto handle = system_->subscribeMouseMove([&](f32, f32, f32, f32) { callCount++; });
...
TEST_F(InputSystemTest, NoModifierMatchesModNone) {
  ...
  auto _ = system_->subscribeKey(Key::W | Action::Press, [&](Key, Action, Mod) { callCount++; });
...
TEST_F(InputSystemTest, UniqueHandleIds) {
  auto h1 = system_->subscribeKey(Key::W | Action::Press, [](Key, Action, Mod) {});
  auto h2 = system_->subscribeKey(Key::S | Action::Press, [](Key, Action, Mod) {});
