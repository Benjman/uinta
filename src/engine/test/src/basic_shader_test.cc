...
TEST_F(BasicShaderTest, ConstructorInitializesUniforms) {
  ...
  gl.onUniform1f = [&](GLint location, GLfloat value) { uniform1fCalls.emplace_back(location, value); };
  gl.onUniform1i = [&](GLint location, GLint value) { uniform1iCalls.emplace_back(location, value); };
  gl.onUniform3f = [&](GLint location, GLfloat v0, GLfloat v1, GLfloat v2) {
    uniform3fCalls.emplace_back(location, v0, v1, v2);
  };
  gl.onUniformMatrix4fv = [&](GLint location, GLsizei, GLboolean, const GLfloat*) {
...
TEST_F(BasicShaderTest, LinkAttributesSetsUpFourAttributes) {
  ...
  gl.onEnableVertexAttribArray = [&](GLuint index) { enabledAttributes.push_back(index); };
  gl.onVertexAttribPointer = [&](GLuint index, GLint, GLenum, GLboolean, GLsizei, const void*) {
  ...
  EXPECT_NE(std::ranges::find(enabledAttributes.begin(), enabledAttributes.end(), 0u), enabledAttributes.end());
  EXPECT_NE(std::ranges::find(enabledAttributes.begin(), enabledAttributes.end(), 1u), enabledAttributes.end());
  EXPECT_NE(std::ranges::find(enabledAttributes.begin(), enabledAttributes.end(), 2u), enabledAttributes.end());
  EXPECT_NE(std::ranges::find(enabledAttributes.begin(), enabledAttributes.end(), 3u), enabledAttributes.end());
}
...
TEST_F(BasicShaderManagerTest, ShaderAccessorReturnsValidPointer) {
  ...
  const BasicShader* constShader = const_cast<const BasicShaderManager&>(manager).shader();
...
TEST_F(DrawElementsCountingTest, TriangleStripAdjacencyCountsCorrectly) {
  ...
  BasicShaderManager manager(&engine);
  manager.drawElements(GL_TRIANGLE_STRIP_ADJACENCY, 10, GL_UNSIGNED_INT, nullptr);
...
TEST_F(FlagMaskMethodsTest, SwayEnablesSwayFlag) {
  ...
  EXPECT_EQ(BasicShaderManager::Masks::Sway, static_cast<u8>(manager.flags()) & BasicShaderManager::Masks::Sway);
}
...
TEST_F(FlagMaskMethodsTest, SwayDisablesSwayFlag) {
  ...
  EXPECT_EQ(0, static_cast<u8>(manager.flags()) & BasicShaderManager::Masks::Sway);
}
...
TEST_F(FlagMaskMethodsTest, UvEnablesUvFlag) {
  ...
  EXPECT_EQ(BasicShaderManager::Masks::Uv, static_cast<u8>(manager.flags()) & BasicShaderManager::Masks::Uv);
}
...
TEST_F(FlagMaskMethodsTest, UvDisablesUvFlag) {
  ...
  EXPECT_EQ(0, static_cast<u8>(manager.flags()) & BasicShaderManager::Masks::Uv);
}
...
TEST_F(FlagMaskMethodsTest, NormalsEnablesNormalsFlag) {
  ...
  EXPECT_EQ(BasicShaderManager::Masks::Normals, static_cast<u8>(manager.flags()) & BasicShaderManager::Masks::Normals);
}
...
TEST_F(FlagMaskMethodsTest, NormalsDisablesNormalsFlag) {
  ...
  EXPECT_EQ(0, static_cast<u8>(manager.flags()) & BasicShaderManager::Masks::Normals);
}
...
TEST_F(FlagMaskMethodsTest, MultipleFlagsCanBeEnabled) {
  ...
  EXPECT_EQ(BasicShaderManager::Masks::Sway, flags & BasicShaderManager::Masks::Sway);
  EXPECT_EQ(BasicShaderManager::Masks::Uv, flags & BasicShaderManager::Masks::Uv);
  EXPECT_EQ(BasicShaderManager::Masks::Normals, flags & BasicShaderManager::Masks::Normals);
}
