...
TEST(DefsTest, EpsilonFIsFloatEpsilon) { EXPECT_EQ(std::numeric_limits<f32>::epsilon(), epsilon_f); }

TEST(DefsTest, EpsilonFIsPositive) { EXPECT_GT(epsilon_f, 0.0f); }

TEST(DefsTest, EpsilonFIsSmall) { EXPECT_LT(epsilon_f, 0.001f); }

TEST(DefsTest, RootThreeFIsCorrect) { EXPECT_FLOAT_EQ(std::sqrt(3.0f), rootThree_f); }

TEST(DefsTest, RootThreeFApproximateValue) { EXPECT_NEAR(1.732050808f, rootThree_f, epsilon_f); }
