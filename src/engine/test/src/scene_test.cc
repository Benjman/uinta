...
TEST_F(SceneTest, FindScene) {
  ...
  struct ChildScene : Scene {
    explicit ChildScene(Engine* engine, SceneLayer layer = SceneLayer::Debug) noexcept : Scene(engine, layer) {}
    explicit ChildScene(Scene* parent, SceneLayer layer = SceneLayer::Debug) noexcept : Scene(parent, layer) {}
  };
  struct LeafScene : Scene {
    explicit LeafScene(Engine* engine, SceneLayer layer = SceneLayer::Debug) noexcept : Scene(engine, layer) {}
    explicit LeafScene(Scene* parent, SceneLayer layer = SceneLayer::Debug) noexcept : Scene(parent, layer) {}
  };
  struct UnusedScene : Scene {
    explicit UnusedScene(Engine* engine, SceneLayer layer = SceneLayer::Debug) noexcept : Scene(engine, layer) {}
    explicit UnusedScene(Scene* parent, SceneLayer layer = SceneLayer::Debug) noexcept : Scene(parent, layer) {}
  };
  ...
  ASSERT_TRUE(foundRoot.has_value()) << "Should find root scene when searching from root.";
  ...
  ASSERT_EQ("uinta::SceneTest_FindScene_Test::TestBody()::ChildScene", child1->name());
  ...
  ASSERT_EQ(child1, foundChild.value()) << "Should return first matching child scene.";
  ASSERT_EQ(&root, foundChild.value()->parent()) << "root expected to be ChildScene's parent.";
  ...
  ASSERT_EQ("uinta::SceneTest_FindScene_Test::TestBody()::LeafScene", leaf1->name());

  // Test finding leaf scene from root (recursive search)
  auto foundLeaf = root.findScene<LeafScene>(true);
  ASSERT_TRUE(foundLeaf.has_value()) << "Should find LeafScene from root with recursive search.";
  ASSERT_EQ(leaf1, foundLeaf.value());
  ASSERT_EQ(child1, foundLeaf.value()->parent()) << "ChildScene expected to be LeafScene's parent.";
  ASSERT_EQ(&root, foundLeaf.value()->parent()->parent()) << "Root expected to be LeafScene's grandparent.";

  // Test finding leaf scene from child
  auto foundLeafFromChild = child1->findScene<LeafScene>();
  ASSERT_TRUE(foundLeafFromChild.has_value()) << "Should find LeafScene from its parent.";
  ...
  ASSERT_TRUE(foundLeafFromSibling.has_value()) << "Should find LeafScene from sibling via root.";
  ...
  ASSERT_FALSE(notFoundNonRecursive.has_value()) << "Should not find LeafScene from sibling without recursive search.";

  // Test finding non-existent scene type
  auto notFound = root.findScene<UnusedScene>();
  ASSERT_FALSE(notFound.has_value()) << "Should not find scene type that doesn't exist in hierarchy.";
}
...
TEST_F(SceneTest, RootSceneExecuting) {
  ...
  scene->onPreRender = [&preRenderCalled](const auto&) { preRenderCalled = true; };

  bool renderCalled = false;
  scene->onRender = [&renderCalled](const auto&) { renderCalled = true; };

  bool postRenderCalled = false;
  scene->onPostRender = [&postRenderCalled](const auto&) { postRenderCalled = true; };
