...
struct MockImageApi : StbImageApi {
  ...
  void setFlipVerticallyOnLoad(
      bool flag_true_if_should_flip) const noexcept override {
  ...
  u8* load(char const* file, i32* x, i32* y, i32* channels_in_file,
           i32 desired_channels) const noexcept override {
    return onLoad(file, x, y, channels_in_file, desired_channels);
  }

  std::function<void(bool)> onImageFree = [](bool) {};
  void imageFree(void* retval_fron_stbi_load) const noexcept override {
    onImageFree(retval_fron_stbi_load != nullptr);
  }
};
