...
class RenderTarget {
 public:
  RenderTarget(const OpenGLApi* gl, u32 width, u32 height, TextureFormat colorFormat,
               std::optional<TextureFormat> depthFormat = std::nullopt, std::string debugName = {}) noexcept;
  ...
  [[nodiscard]] const TextureFormat& colorFormat() const noexcept { return colorFormat_; }

  [[nodiscard]] const std::optional<TextureFormat>& depthFormat() const noexcept { return depthFormat_; }
