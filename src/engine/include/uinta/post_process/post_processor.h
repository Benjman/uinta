...
class PostProcessBeginComponent final : public PreRenderComponent {
 public:
  PostProcessBeginComponent(Engine* /*engine*/, PostProcessor* processor) noexcept : processor_(processor) {}
...
class PostProcessor {
 public:
  ...
  T* addPass(Args&&... args) noexcept {
    static_assert(std::is_base_of_v<PostProcessPass, T>, "T must derive from PostProcessPass");
  ...
  [[nodiscard]] const std::vector<std::unique_ptr<PostProcessPass>>& passes() const noexcept { return passes_; }
  [[nodiscard]] std::vector<std::unique_ptr<PostProcessPass>>& passes() noexcept { return passes_; }
  ...
  [[nodiscard]] const RenderTarget* sceneTarget() const noexcept { return sceneTarget_.get(); }
  ...
  [[nodiscard]] const glm::mat4& projectionMatrix() const noexcept { return projection_; }
