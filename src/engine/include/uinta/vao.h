...
class Vao {
 ...
 public:
  explicit Vao(const OpenGLApi* gl = OpenGLApiImpl::Instance()) noexcept : gl_(gl) { gl_->genVertexArrays(1, &id_); }
  ...
  void linkAttribute(const Attribute& attribute) const noexcept {
    gl_->vertexAttribPointer(attribute.index, attribute.size, attribute.type, attribute.normalized, attribute.stride,
                             std::bit_cast<void*>(attribute.offset));
    gl_->enableVertexAttribArray(attribute.index);
  }

  BufferSegment ebo(const std::span<u32> buffer, const BufferSegment& offset) noexcept {
    return ebo(buffer, offset.offset + offset.size);
  }

  BufferSegment ebo(const std::span<u32> buffer, GLsizeiptr offset = 0) noexcept {
    ...
    return ebo_->bufferData(buffer.data(), static_cast<GLsizeiptr>(size), GL_STATIC_DRAW, offset);
  }
...
class VaoGuard {
 public:
  explicit VaoGuard(const Vao* vao, bool isActive = true) noexcept : id_(vao->id_), gl_(vao->gl_) {
    if (isActive) {
      activate();
    }
  }
