...
class Scene : public RuntimeGetter {
 public:
  class SceneStack {
   ...
   public:
    ...
    T* add(Scene* parent, Args&&... args) noexcept {
      ...
      if (!scene->status_.ok()) {
        LOG(WARNING) << absl::StrFormat("Failed to add scene %s: %s", scene->name(), scene->status_.message());
        return nullptr;
      }
  ...
  virtual std::string name() const noexcept { return demangleTypeName(typeid(*this).name()); }

  const Scene* parent() const noexcept { return parent_; }

  void removeStaleScenes() noexcept {
    std::for_each(children_.stack().begin(), children_.stack().end(), [](auto& scene) { scene->removeStaleScenes(); });
    constexpr auto predicate = [](const auto& scene) { return !(scene->isRunning() || scene->isPaused()); };
    std::erase_if(children_.stack(), predicate);
  }

  const Scene* root() const noexcept { return (parent_ != nullptr) ? parent_->root() : this; }
