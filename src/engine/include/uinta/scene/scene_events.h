...
struct SceneEventPolicies {
  template <typename Event>
  static SceneEvent getEvent(const Event& /*unused*/) {
    return getSceneEvent<Event>();
  }
};

struct SceneDispatchers {
  eventpp::EventDispatcher<SceneEvent, void(const SceneAddedEvent&), SceneEventPolicies> added;

  eventpp::EventDispatcher<SceneEvent, void(const SceneRemovedEvent&), SceneEventPolicies> removed;

  eventpp::EventDispatcher<SceneEvent, void(const SceneStateChangeEvent&), SceneEventPolicies> stateChange;

  eventpp::EventDispatcher<SceneEvent, void(const SceneLayerChangeEvent&), SceneEventPolicies> layerChange;
