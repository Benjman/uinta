...
class Shader {
 public:
  Shader(const std::unordered_map<GLenum, std::string>& stagePaths, const OpenGLApi*) noexcept;
  ...
  [[nodiscard]] const OpenGLApi* gl() const noexcept { return gl_; }

  virtual void linkAttributes(const Vao* /*unused*/) const noexcept {}
...
class ShaderGuard {
 public:
  explicit ShaderGuard(const Shader* shader) noexcept : gl_(shader->gl()) { gl_->useProgram(shader->id()); }
