...
inline f32 getPlaneInterceptDist(const glm::vec3& planePos, const glm::vec3& planeNormal, const glm::vec3& rayPos,
                                 const glm::vec3& rayDirection, f32 minimum = epsilon_f) noexcept;

inline glm::vec3 getPlaneInterceptPoint(const glm::vec3& planePos, const glm::vec3& planeNormal,
                                        const glm::vec3& rayPos, const glm::vec3& rayDirection) noexcept;

inline glm::vec3 getPointOnRay(const glm::vec3& position, const glm::vec3& direction, f32 dist) noexcept;

inline glm::vec3 getRight(f32 yaw) noexcept;

inline glm::vec3 getUp(f32 pitch, f32 yaw) noexcept;

inline glm::vec3 getUp(const glm::vec3& forward, const glm::vec3& right) noexcept;
...
inline glm::vec3 getWorldPoint(glm::vec2 viewport, glm::vec2 cursor, glm::vec3 origin, glm::mat4 view, glm::mat4 proj,
...
inline f32 getPlaneInterceptDist(const glm::vec3& planePos, const glm::vec3& planeNormal, const glm::vec3& rayPos,
                                 const glm::vec3& rayDirection, f32 minimum) noexcept {
  auto denom = glm::dot(planeNormal, rayDirection);
  if (std::abs(denom) <= minimum) {
    return 0;
  }
...
inline glm::vec3 getUp(f32 pitch, f32 yaw) noexcept { return getUp(getForward(pitch, yaw), getRight(yaw)); }

inline glm::vec3 getUp(const glm::vec3& forward, const glm::vec3& right) noexcept { return glm::cross(forward, right); }

inline glm::vec3 getWorldRay(const glm::vec2& cursorPos, const glm::vec2& viewport, const glm::mat4& view,
                             const glm::mat4& proj) noexcept {
  assert((viewport.y != 0 && viewport.x != 0) && "Invalid viewport dimensions.");
  glm::vec3 ndc(((2 * cursorPos.x) / viewport.x) - 1, 1 - ((2 * cursorPos.y) / viewport.y), 1);
