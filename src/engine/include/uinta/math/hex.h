...
struct Hex final {
  ...
  Hex operator=(const glm::ivec2& other) const noexcept { return {other.x, other.y}; }

  Hex operator+(Hex rhs) const noexcept { return {q + rhs.q, r + rhs.r}; }

  Hex operator-(Hex rhs) const noexcept { return {q - rhs.q, r - rhs.r}; }

  bool operator<(const Hex& other) const noexcept { return std::tie(q, r) < std::tie(other.q, other.r); }
  ...
  [[nodiscard]] Hex scale(value_type scale) const noexcept { return {q * scale, r * scale}; }

  static Hex direction(value_type direction) noexcept {
    static const std::array<Hex, 6> directions({Hex(1, 0), Hex(1, -1), Hex(0, -1), Hex(-1, 0), Hex(-1, 1), Hex(0, 1)});
    assert(direction >= 0 && direction < 6 && "Direction must be between 0 and 5.");
    return directions.at(direction);
  }

  static Hex diagonal(value_type direction) noexcept {
    static const std::array<Hex, 6> diagonals({Hex(2, -1), Hex(1, -2), Hex(-1, -1), Hex(-2, 1), Hex(-1, 2), Hex(1, 1)});
    assert(direction >= 0 && direction < 6 && "Direction must be between 0 and 5.");
    return diagonals.at(direction);
  }

  [[nodiscard]] Hex neighbor(value_type direction) const noexcept { return *this + uinta::Hex::direction(direction); }

  [[nodiscard]] value_type length() const noexcept { return static_cast<value_type>((abs(q) + abs(r)) / 2.0); }

  [[nodiscard]] value_type distance(Hex rhs) const noexcept { return (*this - rhs).length(); }

  [[nodiscard]] glm::vec2 centerPoint(f32 size = 1) const noexcept {
    return {size * (1.5 * q), size * ((rootThree_f * 0.5 * q) + (rootThree_f * static_cast<f32>(r)))};
  }

  [[nodiscard]] std::array<glm::vec2, 6> generatePoints(f32 size = 1, f32 fill = 1) const noexcept {
...
struct HexGroup final {
  struct Flags final {
    ...
    [[nodiscard]] bool isRing() const noexcept { return (flags_ & RingMask) != 0; }
    ...
    [[nodiscard]] bool isFlatTop() const noexcept { return (flags_ & OrientationMask) != 0; }
    ...
    [[nodiscard]] bool isLayoutHexagon() const noexcept { return (flags_ & LayoutMask) == LayoutHexagonMask; }
    void isLayoutHexagon(bool v) noexcept { setLayout(LayoutHexagonMask, v); }

    [[nodiscard]] bool isLayoutRectangle() const noexcept { return (flags_ & LayoutMask) == LayoutRectangleMask; }
    void isLayoutRectangle(bool v) noexcept { setLayout(LayoutRectangleMask, v); }

    [[nodiscard]] bool isLayoutRhombus() const noexcept { return (flags_ & LayoutMask) == LayoutRhombusMask; }
    void isLayoutRhombus(bool v) noexcept { setLayout(LayoutRhombusMask, v); }

    [[nodiscard]] bool isLayoutTriangle() const noexcept { return (flags_ & LayoutMask) == LayoutTriangleMask; }
  ...
  HexGroup(Hex origin = {0, 0}, Hex::value_type radius = 2, f32 size = 1, f32 fillPercentage = 1,
           Flags flags = Flags::DefaultFlags) noexcept
      : origin(origin), radius(radius), size(size), fillPercentage(fillPercentage), flags(std::move(flags)) {}
  ...
  [[nodiscard]] glm::vec2 hexToPoint(Hex hex) const noexcept {
    if (flags.isFlatTop()) {
      return {
          size * (3.0 / 2.0 * hex.q),
          size * ((rootThree_f / 2.0 * hex.q) + (rootThree_f * static_cast<f32>(hex.r))),
      };
    }
    return {
        size * ((rootThree_f * static_cast<f32>(hex.q)) + (rootThree_f / 2.0 * hex.r)),
        size * (3.0 / 2.0 * hex.r),
    };
  }
  ...
  [[nodiscard]] Hex pointToHex(glm::vec2 point) const noexcept {
    ...
    if (flags.isFlatTop()) {
      ...
      r = static_cast<f32>((-1.0 / 3.0 * point.x) + ((rootThree_f / 3.0) * point.y)) / size;
    } else {
      q = static_cast<f32>((rootThree_f * point.x) - (1.0 / 3.0 * point.y)) / size;
