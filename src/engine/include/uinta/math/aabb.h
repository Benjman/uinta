...
struct AABB {
  ...
  explicit AABB(std::span<glm::vec3> points) noexcept;
  explicit AABB(std::span<glm::vec2> points) noexcept;
  explicit AABB(std::span<f32> points, size_t components) noexcept;
  ...
  [[nodiscard]] glm::vec3 center() const noexcept { return (min + max) / 2.0f; }

  [[nodiscard]] glm::vec3 size() const noexcept { return {max.x - min.x, max.y - min.y, max.z - min.z}; }
