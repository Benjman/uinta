...
class SmoothFloat {
 public:
  ...
  void update(time_t dt) noexcept {
    ...
    if (fabs(diff) > epsilon_f) {
      current_ += static_cast<f32>(diff * agility_ * dt);
    }
  }
  ...
  bool operator==(const SmoothFloat& other) const noexcept { return fabs(current_ - other.current_) <= epsilon_f; }
  ...
  [[nodiscard]] value_type agility() const noexcept { return agility_; }

  void agility(value_type v) noexcept { agility_ = v; }

  [[nodiscard]] value_type current() const noexcept { return current_; }

  void current(value_type v) noexcept { current_ = v; }

  [[nodiscard]] value_type target() const noexcept { return target_; }
