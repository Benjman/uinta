...
class BasicShader : public Shader {
 ...
 public:
  ...
  void linkAttributes(const Vao* vao) const noexcept override {
    constexpr Attribute position(0, 3, GL_FLOAT, GL_FALSE, Vertex::ElementCount * sizeof(GLfloat), 0 * sizeof(GLfloat));
    constexpr Attribute normal(1, 3, GL_FLOAT, GL_FALSE, Vertex::ElementCount * sizeof(GLfloat), 3 * sizeof(GLfloat));
    constexpr Attribute color(2, 3, GL_FLOAT, GL_FALSE, Vertex::ElementCount * sizeof(GLfloat), 6 * sizeof(GLfloat));
    constexpr Attribute uv(3, 2, GL_FLOAT, GL_FALSE, Vertex::ElementCount * sizeof(GLfloat), 9 * sizeof(GLfloat));
...
class BasicShaderManager : public PreRenderComponent {
 public:
  struct Flags final {
    using value_type = u8;
    ...
    [[nodiscard]] bool isViewDirty() const noexcept { return (flags_ & ViewDirtyMask) != 0; }
    ...
    [[nodiscard]] bool isProjectionDirty() const noexcept { return (flags_ & ProjectionDirtyMask) != 0; }
   ...
   private:
    ...
    static constexpr value_type DefaultFlags = ViewDirtyMask | ProjectionDirtyMask;

    value_type flags_;
  };

  explicit BasicShaderManager(Engine* engine) noexcept;
  ...
  void update(time_t delta) noexcept override {
    ...
    shader_.time = static_cast<f32>(runtime_);
  ...
  void drawElements(GLenum mode, GLsizei count, GLenum type, const void* indices) noexcept {
    ...
    switch (mode) {
      ...
      case GL_LINES_ADJACENCY:
        ...
        break;
      case GL_TRIANGLE_STRIP:  // NOLINT(bugprone-branch-clone)
        renderCount_ += (count - 2);
        break;
  ...
  [[nodiscard]] const BasicShader* shader() const noexcept { return &shader_; }

  [[nodiscard]] count_t renderCount() const noexcept { return renderCount_; }
  ...
  [[nodiscard]] f32 ambientStr() const noexcept { return shader_.ambientStr.value(); }

  void diffuseMin(f32 diffuseMin) noexcept { shader_.diffuseMin = diffuseMin; }
  [[nodiscard]] f32 diffuseMin() const noexcept { return shader_.diffuseMin.value(); }

  void time(f32 time) noexcept { shader_.time = time; }
  [[nodiscard]] f32 time() const noexcept { return shader_.time.value(); }

  void flags(f32 flags) noexcept { shader_.flags = static_cast<GLint>(flags); }
  [[nodiscard]] f32 flags() const noexcept { return static_cast<f32>(shader_.flags.value()); }

  void lightColor(glm::vec3 lightColor) noexcept { shader_.lightColor = lightColor; }
  [[nodiscard]] glm::vec3 lightColor() const noexcept { return shader_.lightColor.value(); }

  void lightDir(glm::vec3 lightDir) noexcept { shader_.lightDir = lightDir; }
  [[nodiscard]] glm::vec3 lightDir() const noexcept { return shader_.lightDir.value(); }

  void model(glm::mat4 model) noexcept { shader_.model = model; }
  [[nodiscard]] glm::mat4 model() const noexcept { return shader_.model.value(); }

  void projection(glm::mat4 projection) noexcept { shader_.projection = projection; }
  [[nodiscard]] glm::mat4 projection() const noexcept { return shader_.projection.value(); }

  void view(glm::mat4 view) noexcept { shader_.view = view; }
  [[nodiscard]] glm::mat4 view() const noexcept { return shader_.view.value(); }

  void linkAttributes(const Vao* vao) const noexcept { shader_.linkAttributes(vao); }
 ...
 private:
  ServiceHandle<BasicShaderManager> serviceHandle_;
  ...
  void setFlag(bool enable, u8 mask) noexcept {
    bool isEnabled = (shader_.flags.value() & mask) != 0;
    if (isEnabled && !enable) {
