...
class BasicShaderManager : public PreRenderComponent {
 public:
  ...
  [[nodiscard]] f32 ambientStr() const noexcept {
    return shader_.ambientStr.value();
  }

  void diffuseMin(f32 diffuseMin) noexcept { shader_.diffuseMin = diffuseMin; }
  [[nodiscard]] f32 diffuseMin() const noexcept {
    return shader_.diffuseMin.value();
  }
  ...
  [[nodiscard]] f32 flags() const noexcept {
    return static_cast<f32>(shader_.flags.value());
  }

  void lightColor(glm::vec3 lightColor) noexcept {
    shader_.lightColor = lightColor;
  }
  [[nodiscard]] glm::vec3 lightColor() const noexcept {
    return shader_.lightColor.value();
  }

  void lightDir(glm::vec3 lightDir) noexcept { shader_.lightDir = lightDir; }
  [[nodiscard]] glm::vec3 lightDir() const noexcept {
    return shader_.lightDir.value();
  }

  void model(glm::mat4 model) noexcept { shader_.model = model; }
  [[nodiscard]] glm::mat4 model() const noexcept {
    return shader_.model.value();
  }
  ...
  [[nodiscard]] glm::mat4 projection() const noexcept {
    return shader_.projection.value();
  }
