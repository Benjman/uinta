...
class Vbo {
 ...
 public:
  ...
  template <typename T>
  BufferSegment bufferData(const T* buffer, GLsizeiptr size, GLenum usage, const BufferSegment& offset) noexcept {
    return bufferData(buffer, size, usage, offset.offset + offset.size);
  }

  template <typename T>
  BufferSegment bufferData(const T* buffer, GLsizeiptr size, GLenum usage, GLintptr offset = 0) noexcept {
    if (offset + size > capacity_) {
      resize(offset + size, usage);
    }
  ...
  [[nodiscard]] GLsizeiptr capacity() const noexcept { return capacity_; }
  ...
  void resize(GLsizeiptr size, GLenum usage) noexcept {
    ...
    if (capacity_ == 0) {
      ...
      gl_->copyBufferSubData(GL_COPY_READ_BUFFER, GL_COPY_WRITE_BUFFER, 0, 0, capacity_);
