...
class GridScene : public Scene {
 public:
  struct Params final {
    ...
    Params(glm::vec2 position = {0, 0}, std::size_t radius = 100,
           Color color = {78 / 255.0, 68 / 255.0, 62 / 255.0, 1.0}, f32 cellWidth = 1.0, f32 cellHeight = 1.0) noexcept
        : position(position), radius(radius), color(color), cellWidth(cellWidth), cellHeight(cellHeight) {}
  };
  ...
  struct Flags final {
    ...
    [[nodiscard]] bool isDepthTest() const noexcept { return (flags_ & DepthTestMask) != 0; }
    ...
    [[nodiscard]] bool isColorDirty() const noexcept { return (flags_ & ColorDirtyMask) != 0; }
 ...
 private:
  struct GridShader : public Shader {
    ...
    void linkAttributes(const Vao* vao) const noexcept override {
      constexpr Attribute PositionAttrib(0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), 0);
