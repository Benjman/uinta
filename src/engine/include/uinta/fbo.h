...
class Fbo {
 ...
 public:
  struct Flags final {
    ...
    [[nodiscard]] bool hasDepthAttachment() const noexcept { return (flags_ & DepthAttachmentMask) != 0; }
    ...
    [[nodiscard]] bool hasDepthStencilAttachment() const noexcept { return (flags_ & DepthStencilAttachmentMask) != 0; }
  ...
  explicit Fbo(const OpenGLApi* gl = OpenGLApiImpl::Instance()) noexcept : gl_(gl) { gl_->genFramebuffers(1, &id_); }
  ...
  void attachColorTexture(const Texture* texture, GLuint colorAttachment = 0, GLint level = 0) noexcept {
    gl_->framebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + colorAttachment, texture->target(), texture->id(),
                              level);
  ...
  void attachDepthTexture(const Texture* texture, GLint level = 0) noexcept {
    gl_->framebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, texture->target(), texture->id(), level);
    flags_.hasDepthAttachment(true);
  }

  // Attach a texture to the depth-stencil attachment point
  void attachDepthStencilTexture(const Texture* texture, GLint level = 0) noexcept {
    gl_->framebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, texture->target(), texture->id(), level);
    flags_.hasDepthStencilAttachment(true);
  }

  // Attach a renderbuffer to a color attachment point
  void attachColorRenderbuffer(GLuint renderbuffer, GLuint colorAttachment = 0) noexcept {
    gl_->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + colorAttachment, GL_RENDERBUFFER, renderbuffer);
    if (colorAttachment >= colorAttachments_.size()) {
      colorAttachments_.resize(colorAttachment + 1);
    }
    colorAttachments_[colorAttachment] = GL_COLOR_ATTACHMENT0 + colorAttachment;
  ...
  void attachDepthRenderbuffer(GLuint renderbuffer) noexcept {
    gl_->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, renderbuffer);
    flags_.hasDepthAttachment(true);
  }
  ...
  void attachDepthStencilRenderbuffer(GLuint renderbuffer) noexcept {
    gl_->framebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, renderbuffer);
    flags_.hasDepthStencilAttachment(true);
  }
  ...
  void setDrawBuffers() const noexcept {
    if (!colorAttachments_.empty()) {
      gl_->drawBuffers(static_cast<GLsizei>(colorAttachments_.size()), colorAttachments_.data());
    }
  }

  // Check if the framebuffer is complete
  [[nodiscard]] GLenum checkStatus() const noexcept { return gl_->checkFramebufferStatus(GL_FRAMEBUFFER); }

  // Check if the framebuffer is complete and return success status
  [[nodiscard]] bool isComplete() const noexcept { return checkStatus() == GL_FRAMEBUFFER_COMPLETE; }
...
class FboGuard {
 public:
  explicit FboGuard(const Fbo* fbo, bool isActive = true) noexcept : id_(fbo->id_), gl_(fbo->gl_) {
    if (isActive) {
      activate();
    }
  }
