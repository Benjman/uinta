...
class Camera {
 public:
  ...
  [[nodiscard]] const SmoothFloat& angle() const noexcept { return config_.angle; }

  void angle(f32 angle) noexcept { config_.angle = angle; }

  void angle(SmoothFloat angle) noexcept { config_.angle = angle; }

  void enableKeyboard(bool v = true) noexcept { config_.flags.isKeyboardEnabled(v); }
  ...
  [[nodiscard]] const SmoothFloat& dist() const noexcept { return config_.dist; }

  void dist(f32 dist) noexcept { config_.dist = dist; }

  void dist(SmoothFloat dist) noexcept { config_.dist = dist; }

  [[nodiscard]] CameraConfig::Flags flags() const noexcept { return config_.flags; }

  CameraConfig::Flags& flags() noexcept { return config_.flags; }

  void flags(CameraConfig::Flags flags) noexcept { config_.flags = std::move(flags); }

  [[nodiscard]] const SmoothFloat& pitch() const noexcept { return config_.pitch; }
  ...
  [[nodiscard]] const SmoothVec3& target() const noexcept { return config_.target; }
