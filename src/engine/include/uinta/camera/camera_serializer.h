...
inline CameraConfig deserialize(const AppConfig* appConfig) noexcept {
  auto getSmoothFloat = [&](const std::string& key, f32 fallbackValue, f32 fallbackAgility) -> SmoothFloat {
    return SmoothFloat(appConfig->getFloat(key + ".agility").value_or(fallbackAgility),
                       appConfig->getFloat(key + ".value").value_or(fallbackValue));
  };

  auto getSmoothVec3 = [&](const std::string& key, f32 fallbackX, f32 fallbackY, f32 fallbackZ,
                           f32 fallbackAgility) -> SmoothVec3 {
    ...
    f32 agility = appConfig->getFloat(key + ".agility").value_or(fallbackAgility);
  ...
  config.spdFactorMin = appConfig->getFloat("camera.speedFactorMin").value_or(0.66);
  config.trnslScaleFct = appConfig->getFloat("camera.translationScaleFactor").value_or(0.3);
  config.vertOff = appConfig->getFloat("camera.verticalOffset").value_or(0);

  return config;
}

inline CameraKeybindings deserializeKeybindings(const AppConfig* appConfig) noexcept {
  auto getInputKey = [&](const std::string& key, InputKey fallback) -> InputKey {
    if (auto opt = appConfig->getString(key); opt.has_value()) {
      if (auto keyOpt = Input::GetKeyFromStr(opt.value()); keyOpt.has_value()) {
        return keyOpt.value();
      }
      LOG(WARNING) << absl::StrFormat("Could not find InputKey from %s.", opt.value());
    }
    return fallback;
  };

  auto getMouseButton = [&](const std::string& key, MouseButton fallback) -> MouseButton {
    if (auto opt = appConfig->getString(key); opt.has_value()) {
      if (auto keyOpt = Input::GetMouseButtonFromStr(opt.value()); keyOpt.has_value()) {
        return keyOpt.value();
      }
      LOG(WARNING) << absl::StrFormat("Could not find MouseButton from %s.", opt.value());
    }
    return fallback;
  };
  ...
  kb.trnslM = getMouseButton("camera.mappings.mouse.translation", MOUSE_BUTTON_MIDDLE);
  ...
  kb.trnslSpdK = appConfig->getFloat("camera.translationSpeedKeyboard").value_or(10);
  kb.trnslSpdM = appConfig->getFloat("camera.translationSpeedMouse").value_or(2);
...
inline void serialize(AppConfig* appConfig, CameraConfig config) noexcept {
  ...
  auto setSmoothFloat = [&](SmoothFloat* value, const std::string& appConfigKey) -> void {
    setFloat(value->target(), appConfigKey + ".value");
    setFloat(value->agility(), appConfigKey + ".agility");
  };

  auto setSmoothVec3 = [&](SmoothVec3* value, const std::string& appConfigKey) -> void {
  ...
  LOG(INFO) << absl::StrFormat(
      "Camera properties serialized: target=(%.2f,%.2f,%.2f), distance=%.2f, pitch=%.2f, angle=%.2f",
      config.target.x().target(), config.target.y().target(), config.target.z().target(), config.dist.target(),
      config.pitch.target(), config.angle.target());
}

inline void serializeKeybindings(AppConfig* appConfig, const CameraKeybindings& kb) noexcept {
  auto setInputKey = [&](InputKey inputKey, const std::string& appConfigKey) -> void {
    if (auto opt = Input::GetKeyStr(inputKey); opt.has_value()) {
      if (auto status = appConfig->setString(appConfigKey, opt.value()); !status.ok()) {
        LOG(WARNING) << status.message();
      }
    }
  };

  auto setMouseButton = [&](MouseButton button, const std::string& appConfigKey) -> void {
