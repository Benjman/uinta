...
class Mesh {
 public:
  ...
  [[nodiscard]] std::span<const Vertex> verticesc() const noexcept {
    return vertices_;
  }

  [[nodiscard]] const std::vector<idx_t>& elements() const noexcept {
    return elements_;
  }

  std::span<idx_t> elements() noexcept { return elements_; }

  [[nodiscard]] std::span<const idx_t> elementsc() const noexcept {
    return elements_;
  }
  ...
  struct Hexagon final {
    static Mesh Fill(const Hex*, f32 spaceRad = 1, f32 fill = 1.0,
                     Vertex::color_type color = {1, 1, 1}) noexcept;
    static Mesh Fill(const HexGroup*) noexcept;
    static Mesh Outline(const Hex*, f32 radius = 1, f32 fill = 1.0,
                        Vertex::color_type color = {1, 1, 1}) noexcept;
