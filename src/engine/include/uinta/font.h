...
class Font {
 public:
  explicit Font(const std::string& path, i32 bmapWidth = 1024, i32 bmapHeight = 1024, f32 bmapLineSize = 128,
  ...
  StatusOr<stbtt_aligned_quad> quad(i8 c, f32* x, f32* y, f32 scale = 1) const noexcept;
...
class TextMeshBuilder {
 ...
 public:
  ...
  TextMeshBuilder& dimensions(const glm::vec2& dimensions) noexcept {
    dimensions_.x = (dimensions.x != 0.0f) ? dimensions.x : std::numeric_limits<f32>::max();
    dimensions_.y = (dimensions.y != 0.0f) ? dimensions.y : std::numeric_limits<f32>::max();
    return *this;
  }
 ...
 private:
  struct Word {
    ...
    [[nodiscard]] f32 calculateWidth() const noexcept {
      f32 width = 0;
      std::ranges::for_each(quads.begin(), quads.end(), [&width](auto& quad) { width += quad.x1 - quad.x0; });
      return width;
    }
  };

  struct TextMeshGenerator {
    explicit TextMeshGenerator(const TextMeshBuilder* builder) noexcept : builder_(builder) {}
  ...
  struct BottomCenterTextMeshGenerator : TextMeshGenerator {
    explicit BottomCenterTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  struct BottomLeftTextMeshGenerator : TextMeshGenerator {
    explicit BottomLeftTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  struct BottomRightTextMeshGenerator : TextMeshGenerator {
    explicit BottomRightTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  struct MiddleCenterTextMeshGenerator : TextMeshGenerator {
    explicit MiddleCenterTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  struct MiddleLeftTextMeshGenerator : TextMeshGenerator {
    explicit MiddleLeftTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  struct MiddleRightTextMeshGenerator : TextMeshGenerator {
    explicit MiddleRightTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  struct TopCenterTextMeshGenerator : TextMeshGenerator {
    explicit TopCenterTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  struct TopLeftTextMeshGenerator : TextMeshGenerator {
    explicit TopLeftTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  struct TopRightTextMeshGenerator : TextMeshGenerator {
    explicit TopRightTextMeshGenerator(const TextMeshBuilder* builder) noexcept : TextMeshGenerator(builder) {}

    std::vector<f32>& generate(const std::vector<Word>& /*unused*/) noexcept override;
  };

  friend std::unique_ptr<TextMeshBuilder::TextMeshGenerator> findMeshGenerator(const TextMeshBuilder*) noexcept;

  std::string text_;
  glm::vec4 color_ = glm::vec4(1);
  glm::vec2 dimensions_ = glm::vec2(std::numeric_limits<f32>::max());
  glm::vec2 position_ = {0, 0};
  Flags flags_ = (_::Masks::Top << _::Shifts::Vertical) | (_::Masks::Left << _::Shifts::Horizontal);
  f32 opacity_ = 1;
  f32 lineSize_;
  const Font* font_;

  [[nodiscard]] bool isLeftAligned() const noexcept {
    return ((flags_ & _::Masks::Horizontal) >> _::Shifts::Horizontal) == _::Masks::Left;
  }

  [[nodiscard]] bool isRightAligned() const noexcept {
    return ((flags_ & _::Masks::Horizontal) >> _::Shifts::Horizontal) == _::Masks::Right;
  }

  [[nodiscard]] bool isCenterAligned() const noexcept {
    return ((flags_ & _::Masks::Horizontal) >> _::Shifts::Horizontal) == _::Masks::Center;
  }

  [[nodiscard]] bool isTopAligned() const noexcept {
    return ((flags_ & _::Masks::Vertical) >> _::Shifts::Vertical) == _::Masks::Top;
  }

  [[nodiscard]] bool isMiddleAligned() const noexcept {
    return ((flags_ & _::Masks::Vertical) >> _::Shifts::Vertical) == _::Masks::Middle;
  }

  [[nodiscard]] bool isBottomAligned() const noexcept {
    return ((flags_ & _::Masks::Vertical) >> _::Shifts::Vertical) == _::Masks::Bottom;
  }
};
