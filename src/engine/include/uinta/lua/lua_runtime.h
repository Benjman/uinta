...
class LuaRuntime {
 public:
  struct Flags final {
    ...
    [[nodiscard]] bool configReadEnabled() const noexcept { return (flags_ & ConfigReadMask) != 0; }
    void configReadEnabled(bool v) noexcept { flags_ = (flags_ & ~ConfigReadMask) | (v ? ConfigReadMask : 0); }

    /// Whether plugins can write to AppConfig.
    [[nodiscard]] bool configWriteEnabled() const noexcept { return (flags_ & ConfigWriteMask) != 0; }
    void configWriteEnabled(bool v) noexcept { flags_ = (flags_ & ~ConfigWriteMask) | (v ? ConfigWriteMask : 0); }

    /// Whether hot reload is enabled.
    [[nodiscard]] bool hotReloadEnabled() const noexcept { return (flags_ & HotReloadMask) != 0; }
    void hotReloadEnabled(bool v) noexcept { flags_ = (flags_ & ~HotReloadMask) | (v ? HotReloadMask : 0); }
  ...
  /// @return Status indicating success or failure.
  [[nodiscard]] Status loadPlugins(const std::vector<std::filesystem::path>& searchPaths);
  ...
  /// @return Status indicating success or failure.
  [[nodiscard]] Status executeScript(const std::string& script, const std::string& chunkName = "chunk");
  ...
  [[nodiscard]] static constexpr ApiVersion apiVersion() noexcept { return CURRENT_LUA_API_VERSION; }
