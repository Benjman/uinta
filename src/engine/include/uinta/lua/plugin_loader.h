...
class PluginLoader {
 public:
  ...
  [[nodiscard]] StatusOr<PluginInfo> loadPluginInfo(const std::string& pluginPath) const;
  ...
  [[nodiscard]] StatusOr<std::vector<std::string>> resolveLoadOrder(const std::vector<PluginInfo>& plugins) const;
};
