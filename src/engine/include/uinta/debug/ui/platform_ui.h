...
inline void RenderPlatformUi(Platform* platform) noexcept {
  ...
  ImGui::SeparatorText(absl::StrFormat("Monitor%s", (platform->monitors().size() != 0u) ? "s" : "").c_str());
  for (const auto& monitor : platform->monitors()) {
    ImGui::Text("%-10s %u x %u  @%uhz", monitor.name().c_str(), monitor.height(), monitor.width(), monitor.hz());
  }

  ImGui::SeparatorText(absl::StrFormat("Window \"%s\"", window->name()).c_str());
  ImGui::Text("%-13s %s", "Monitor", window->monitor()->name().c_str());
  ImGui::Text("%-13s %u x %u", "Position", window->x(), window->y());
  ImGui::Text("%-13s %u x %u  (%.3f aspect)", "Resolution", window->height(), window->width(), window->aspect());
