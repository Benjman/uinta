...
inline std::string formatDuration(absl::Duration duration, u8 flags = 0) noexcept {
  ...
  auto hours = (total_seconds / (static_cast<i64>(60 * 60))) % 24;

  auto milliseconds = nanoseconds / absl::Nanoseconds(1000000);
  auto microseconds = (nanoseconds % absl::Nanoseconds(1000000)) / absl::Nanoseconds(1000);

  std::string result;

  if (hours != 0) {
    result = absl::StrCat(hours, "h ");
  }
  if (minutes != 0) {
    result = absl::StrCat(result, minutes, "m ");
  }
  result = absl::StrCat(result, absl::StrFormat("%d", seconds));
  if ((flags & OmitMiilliseconds) == 0) {
    result = absl::StrCat(result, absl::StrFormat(".%03d", milliseconds));
  }
  if ((flags & OmitMicroseconds) == 0) {
