...
inline void RenderPostProcessUi(PostProcessor* processor) noexcept {
  ...
  ImGui::Text("Target: %u x %u (HDR RGBA16F)", processor->width(), processor->height());
  ...
  if (passes.empty()) {
    ImGui::TextWrapped(
        "No passes registered. The simulation color target is being blitted directly to the default framebuffer.");
    ImGui::TreePop();
    return;
  }
