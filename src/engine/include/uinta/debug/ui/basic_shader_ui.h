...
struct Flags final {
  ...
  [[nodiscard]] bool isNormals() const noexcept { return (flags_ & Normals) != 0u; }
  ...
  [[nodiscard]] bool isPolygons() const noexcept { return (flags_ & Polygons) != 0u; }
...
inline void RenderBasicShaderUi(BasicShaderManager* bsm, const OpenGLApi* gl) noexcept {
  if (!ImGui::TreeNode("Basic shader")) {
    return;
  }
  ...
  if (ImGui::ColorEdit3("Ambient Color##BasicShaderAmbientColor", glm::value_ptr<f32>(color))) {
    ShaderGuard guard(bsm->shader());
    bsm->lightColor(color);
  }

  ImGui::PushItemWidth(static_cast<f32>(UiHalfWidth - (UiSpacing * 0.5)));
  auto diffuseMin = bsm->diffuseMin();
  if (ImGui::SliderFloat("##BasicShaderDiffuseMin", &diffuseMin, 0, 1, "Diffuse Min %.2f")) {
    ShaderGuard guard(bsm->shader());
  ...
  if (ImGui::SliderFloat("##BasicShaderAmbientStr", &ambientStr, 0, 1, "Ambient Str %.2f")) {
    ShaderGuard guard(bsm->shader());
  ...
  updateLightDir =
      ImGui::SliderAngle("##BasicShaderLightDirAzimuth", &azimuth, -180.0f, 180.0f, "Azimuth %.2f") || updateLightDir;

  ImGui::SameLine();

  updateLightDir = ImGui::SliderAngle("##BasicShaderLightDirElevation", &elevation, -90.0f, 90.0f, "Elevation %.2f") ||
                   updateLightDir;
  ImGui::PopItemWidth();

  if (updateLightDir) {
    ShaderGuard guard(bsm->shader());
    bsm->lightDir(glm::vec3(cos(elevation) * cos(azimuth), sin(elevation), cos(elevation) * sin(azimuth)));
  }
