...
inline void RenderCameraUi(CameraManager* camManager,
                           ViewportManager* viewport) noexcept {
  ...
  if (camera->flags().isDistLimit()) {
    ImGui::PushItemWidth(UiWidth - ImGui::GetItemRectSize().x -
                         (UiSpacing * 2) - UiOneQuartersWidth);
  ...
  ImGui::PushItemWidth(UiWidth - ImGui::GetItemRectSize().x - UiSpacing);
  ImGui::SameLine();
  if (ImGui::DragFloat2("##CameraClippingPlanes", cameraClippingValues.data(),
                        0.05, farPlane, nearPlane, "%.1f",
  ...
  if (static_cast<ProjectionType>(projectionSelected) ==
