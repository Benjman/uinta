...
inline void RenderEngineUi(const EngineUiInfo& info) noexcept {
  ...
  ImGui::Text("%s %s", "Runtime",
              formatDuration(absl::Seconds(state.runtime()), OmitMiilliseconds | OmitMicroseconds).c_str());
  ...
  ImGui::Text("Tick %s", formatDuration(absl::Seconds(tick.getAverage())).c_str());
  ImGui::PopItemWidth();
  ImGui::SameLine();
  ...
  ImGui::PushItemWidth(UiOneQuartersWidth);
  ImGui::Text("Render %s", formatDuration(absl::Seconds(render.getAverage())).c_str());
  ImGui::PopItemWidth();
  ImGui::SameLine();
  std::ranges::copy(render, container.begin());
  ImGui::PushItemWidth(UiWidth - ImGui::GetItemRectSize().x - UiSpacing);
  ImGui::PlotLines("##Render", container.data(), static_cast<i32>(render.size()));
  ImGui::PopItemWidth();
