...
class PreTickComponent : public Component {
 public:
  ...
  void update(time_t /*unused*/) noexcept override = 0;
};
...
class IntervalComponent : public Component {
 public:
  ...
  explicit IntervalComponent(time_t interval, bool isOneShot = false) noexcept : interval_(interval) {
    assert(interval > 0);
  ...
  [[nodiscard]] time_t interval() const noexcept { return interval_; }

  [[nodiscard]] time_t nextExecutionTime() const noexcept { return nextExecutionTime_; }

  [[nodiscard]] bool isComplete() const noexcept { return flags_.isTerminate(); }
 ...
 private:
  ...
  struct Flags final {
    ...
    [[nodiscard]] bool isOneShot() const noexcept { return (flags_ & OneShotMask) != 0; }
    ...
    [[nodiscard]] bool isTerminate() const noexcept { return (flags_ & TerminateMask) != 0; }
...
class ComponentManager {
 public:
  explicit ComponentManager(const RuntimeGetter* runtimeGetter) noexcept : runtimeGetter_(runtimeGetter) {
  ...
  struct CompareNextExecutionTime {
    bool operator()(const std::unique_ptr<IntervalComponent>& a,
  ...
  using IntervalComponentQueue =
      std::priority_queue<std::unique_ptr<IntervalComponent>, std::vector<std::unique_ptr<IntervalComponent>>,
                          CompareNextExecutionTime>;

  template <Component::Type S>
  using component_type_to_class_t = typename Component::component_type_to_class<S>::type;

  template <typename T, typename... Args>
  T* add(Args&&... args) noexcept {
    static_assert(std::is_base_of_v<Component, T>);
    static_assert(std::is_base_of_v<component_type_to_class_t<T::Type_>, T>, "Component does not match its type");
    if constexpr (T::Type_ == Component::Type::PreTick) {
      absl::MutexLock lock(preTickComponentsMtx_);
      return reinterpret_cast<T*>(
          preTickComponents_.emplace_back(std::make_unique<T>(std::forward<Args>(args)...)).get());
    } else if constexpr (T::Type_ == Component::Type::Tick) {
      absl::MutexLock lock(tickComponentsMtx_);
      return reinterpret_cast<T*>(tickComponents_.emplace_back(std::make_unique<T>(std::forward<Args>(args)...)).get());
    } else if constexpr (T::Type_ == Component::Type::PostTick) {
      absl::MutexLock lock(postTickComponentsMtx_);
      return reinterpret_cast<T*>(
          postTickComponents_.emplace_back(std::make_unique<T>(std::forward<Args>(args)...)).get());
    } else if constexpr (T::Type_ == Component::Type::PreRender) {
      absl::MutexLock lock(preRenderComponentsMtx_);
      return reinterpret_cast<T*>(
          preRenderComponents_.emplace_back(std::make_unique<T>(std::forward<Args>(args)...)).get());
    } else if constexpr (T::Type_ == Component::Type::Render) {
      absl::MutexLock lock(renderComponentsMtx_);
      return reinterpret_cast<T*>(
          renderComponents_.emplace_back(std::make_unique<T>(std::forward<Args>(args)...)).get());
    } else if constexpr (T::Type_ == Component::Type::PostRender) {
      absl::MutexLock lock(postRenderComponentsMtx_);
      return reinterpret_cast<T*>(
          postRenderComponents_.emplace_back(std::make_unique<T>(std::forward<Args>(args)...)).get());
    ...
    if constexpr (T::Type_ == Component::Type::PreTick) {
      auto itr =
          std::find_if(preTickComponents_.begin(), preTickComponents_.end(),
                       [component](const std::unique_ptr<PreTickComponent>& ptr) { return ptr.get() == component; });
      ...
      auto itr =
          std::find_if(tickComponents_.begin(), tickComponents_.end(),
                       [component](const std::unique_ptr<TickComponent>& ptr) { return ptr.get() == component; });
      ...
      auto itr =
          std::find_if(postTickComponents_.begin(), postTickComponents_.end(),
                       [component](const std::unique_ptr<PostTickComponent>& ptr) { return ptr.get() == component; });
      ...
      auto itr =
          std::find_if(preRenderComponents_.begin(), preRenderComponents_.end(),
                       [component](const std::unique_ptr<PreRenderComponent>& ptr) { return ptr.get() == component; });
      ...
      auto itr =
          std::find_if(renderComponents_.begin(), renderComponents_.end(),
                       [component](const std::unique_ptr<RenderComponent>& ptr) { return ptr.get() == component; });
      ...
      auto itr =
          std::find_if(postRenderComponents_.begin(), postRenderComponents_.end(),
                       [component](const std::unique_ptr<PostRenderComponent>& ptr) { return ptr.get() == component; });
      ...
      while (!intervalComponentQueue_.empty()) {
        auto top = std::move(const_cast<std::unique_ptr<IntervalComponent>&>(intervalComponentQueue_.top()));
  ...
  std::optional<T*> find() const noexcept {
    ...
    if constexpr (T::Type_ == Component::Type::PostRender) {
      if (auto itr = std::find_if(postRenderComponents_.begin(), postRenderComponents_.end(),
                                  [](const auto& component) { return dynamic_cast<const T*>(component.get()); });
          itr != postRenderComponents_.end()) {
        return reinterpret_cast<T*>(itr->get());
      }
    } else if constexpr (T::Type_ == Component::Type::PostTick) {
      if (auto itr = std::find_if(postTickComponents_.begin(), postTickComponents_.end(),
                                  [](const auto& component) { return dynamic_cast<const T*>(component.get()); });
          itr != postTickComponents_.end()) {
        return reinterpret_cast<T*>(itr->get());
      }
    } else if constexpr (T::Type_ == Component::Type::PreRender) {
      if (auto itr = std::find_if(preRenderComponents_.begin(), preRenderComponents_.end(),
                                  [](const auto& component) { return dynamic_cast<const T*>(component.get()); });
          itr != preRenderComponents_.end()) {
        return reinterpret_cast<T*>(itr->get());
      }
    } else if constexpr (T::Type_ == Component::Type::PreTick) {
      if (auto itr = std::find_if(preTickComponents_.begin(), preTickComponents_.end(),
                                  [](const auto& component) { return dynamic_cast<const T*>(component.get()); });
          itr != preTickComponents_.end()) {
        return reinterpret_cast<T*>(itr->get());
      }
    } else if constexpr (T::Type_ == Component::Type::Render) {
      if (auto itr = std::find_if(renderComponents_.begin(), renderComponents_.end(),
                                  [](const auto& component) { return dynamic_cast<const T*>(component.get()); });
          itr != renderComponents_.end()) {
        return reinterpret_cast<T*>(itr->get());
      }
    } else if constexpr (T::Type_ == Component::Type::Tick) {
      if (auto itr = std::find_if(tickComponents_.begin(), tickComponents_.end(),
                                  [](const auto& component) { return dynamic_cast<const T*>(component.get()); });
  ...
  void update(time_t delta) noexcept {
    constexpr std::array<Component::Type, 7> EngineStageToType = {
        Component::Type::PreTick,   Component::Type::Tick,   Component::Type::PostTick,
        Component::Type::PreRender, Component::Type::Render, Component::Type::PostRender,
    };
  ...
  void update(time_t delta) noexcept {
    runtime_ += delta;
    updateIntervalComponents(delta);

    if constexpr (S == Component::Type::PreTick) {
      std::for_each(preTickComponents_.begin(), preTickComponents_.end(),
                    [delta](auto& component) noexcept { component->update(delta); });
    } else if constexpr (S == Component::Type::Tick) {
      std::for_each(tickComponents_.begin(), tickComponents_.end(),
                    [delta](auto& component) noexcept { component->update(delta); });
    } else if constexpr (S == Component::Type::PostTick) {
      std::for_each(postTickComponents_.begin(), postTickComponents_.end(),
                    [delta](auto& component) noexcept { component->update(delta); });
    } else if constexpr (S == Component::Type::PreRender) {
      std::for_each(preRenderComponents_.begin(), preRenderComponents_.end(),
                    [delta](auto& component) noexcept { component->update(delta); });
    } else if constexpr (S == Component::Type::Render) {
      std::for_each(renderComponents_.begin(), renderComponents_.end(),
                    [delta](auto& component) noexcept { component->update(delta); });
    } else if constexpr (S == Component::Type::PostRender) {
      std::for_each(postRenderComponents_.begin(), postRenderComponents_.end(),
                    [delta](auto& component) noexcept { component->update(delta); });
    }
  }

  [[nodiscard]] size_t allSize() const noexcept {
    // clang-format off
  ...
  [[nodiscard]] const ComponentStorage<PreTickComponent>& preTickComponents() const noexcept {
    return preTickComponents_;
  }

  [[nodiscard]] const ComponentStorage<TickComponent>& tickComponents() const noexcept { return tickComponents_; }

  [[nodiscard]] const ComponentStorage<PostTickComponent>& postTickComponents() const noexcept {
    return postTickComponents_;
  }

  [[nodiscard]] const ComponentStorage<PreRenderComponent>& preRenderComponents() const noexcept {
    return preRenderComponents_;
  }

  [[nodiscard]] const ComponentStorage<RenderComponent>& renderComponents() const noexcept { return renderComponents_; }

  [[nodiscard]] const ComponentStorage<PostRenderComponent>& postRenderComponents() const noexcept {
    return postRenderComponents_;
  }

  [[nodiscard]] const IntervalComponentQueue& intervalComponentQueue() const noexcept {
 ...
 private:
  ...
  void updateIntervalComponents(time_t delta) noexcept {
    if (intervalComponentQueue_.empty()) {
      return;
    }
    while (!intervalComponentQueue_.empty() && runtime_ >= intervalComponentQueue_.top()->nextExecutionTime()) {
      auto component = std::move(const_cast<std::unique_ptr<IntervalComponent>&>(intervalComponentQueue_.top()));
      ...
      if (!component->isComplete()) {
        ...
        while ((component->nextExecutionTime_ += static_cast<time_t>(component->interval_ < runtime_)) != 0.0) {
          missedIntervalCount++;
        }
        if (missedIntervalCount != 0u) {
          LOG(WARNING) << absl::StrFormat("Interval component missed %u interval calls.", missedIntervalCount);
        }
        absl::MutexLock lock(intervalComponentQueueMtx_);
