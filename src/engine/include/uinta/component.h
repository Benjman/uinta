...
class IntervalComponent : public Component {
 public:
  ...
  [[nodiscard]] time_t nextExecutionTime() const noexcept {
    return nextExecutionTime_;
  }

  [[nodiscard]] bool isComplete() const noexcept {
    return flags_.isTerminate();
  }
 ...
 private:
  ...
  struct Flags final {
    ...
    [[nodiscard]] bool isTerminate() const noexcept {
      return (flags_ & TerminateMask) != 0;
    }
...
class ComponentManager {
 public:
  ...
  [[nodiscard]] const ComponentStorage<PreTickComponent>& preTickComponents()
      const noexcept {
    return preTickComponents_;
  }

  [[nodiscard]] const ComponentStorage<TickComponent>& tickComponents()
      const noexcept {
    return tickComponents_;
  }

  [[nodiscard]] const ComponentStorage<PostTickComponent>& postTickComponents()
      const noexcept {
    return postTickComponents_;
  }

  [[nodiscard]] const ComponentStorage<PreRenderComponent>&
  preRenderComponents() const noexcept {
    return preRenderComponents_;
  }

  [[nodiscard]] const ComponentStorage<RenderComponent>& renderComponents()
      const noexcept {
    return renderComponents_;
  }

  [[nodiscard]] const ComponentStorage<PostRenderComponent>&
  postRenderComponents() const noexcept {
    return postRenderComponents_;
  }

  [[nodiscard]] const IntervalComponentQueue& intervalComponentQueue()
      const noexcept {
 ...
 private:
  ...
  void updateIntervalComponents(time_t delta) noexcept {
    ...
    while (!intervalComponentQueue_.empty() &&
           runtime_ >= intervalComponentQueue_.top()->nextExecutionTime()) {
      auto component =
      ...
      if (!component->isComplete()) {
        ...
        while ((component->nextExecutionTime_ +=
                static_cast<time_t>(component->interval_ < runtime_)) != 0.0) {
          missedIntervalCount++;
        }
