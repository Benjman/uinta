...
class Texture {
 ...
 public:
   ...
   * mutable storage). */
  Texture(GLenum target = GL_TEXTURE_2D, i32 width = 0, i32 height = 0, i32 channels = 0, i32 filter = 0, i32 wrap = 0,
  ...
  Texture(GLenum target, i32 width, i32 height, const TextureFormat& format, i32 filterMin, i32 filterMag, i32 wrap,
          const OpenGLApi* = OpenGLApiImpl::Instance()) noexcept;
  ...
  Status fromFile(const std::filesystem::path& path,
...
class ActiveTextureGuard {
 public:
  explicit ActiveTextureGuard(GLenum texture = GL_TEXTURE0, const OpenGLApi* gl = OpenGLApiImpl::Instance()) noexcept
      : gl_(gl) {
