...
class Texture {
 ...
 public:
  Texture(GLenum target = GL_TEXTURE_2D, i32 width = 0, i32 height = 0,
          i32 channels_ = 0, i32 filter = 0, i32 wrap = 0,
...
class TextureGuard {
 public:
  explicit TextureGuard(const Texture* texture) noexcept
      : target_(texture->target_), gl_(texture->gl_) {
    gl_->bindTexture(target_, texture->id());
  }

  ~TextureGuard() noexcept { gl_->bindTexture(target_, 0); }
