...
class ServiceHandle {
 public:
  ServiceHandle(Engine* engine, T* service) noexcept : engine_(engine) { engine_->registerService<T>(service); }
  ...
  ServiceHandle(ServiceHandle&& other) noexcept : engine_(std::exchange(other.engine_, nullptr)) {}
