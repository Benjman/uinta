...
class CursorManager final : public PreRenderComponent {
 public:
  CursorManager(Engine* engine) noexcept
    ...
    cameraManager->addListener<CameraEvent::ViewMatrixUpdated>([this](const auto&) { updateCursorWorldPoint(); });
  ...
  void updateCursorWorldPoint() const noexcept {
    ...
    input_->cursorWorldPoint(getWorldPoint(viewport, cursorPos_, camera_->position(), view, projection_));
  }
