...
Mesh::Mesh(const std::span<const Mesh> collection) noexcept {
  std::ranges::for_each(collection.begin(), collection.end(), [this](auto& mesh) {
    vertices_.insert(vertices_.end(), mesh.vertices().begin(), mesh.vertices().end());
    elements_.insert(elements_.end(), mesh.elements().begin(), mesh.elements().end());
  });
}
...
Mesh::Mesh(const Mesh& other) noexcept = default;
...
Mesh::Mesh(Mesh&& other) noexcept : vertices_(std::move(other.vertices_)), elements_(std::move(other.elements_)) {}
...
void Mesh::merge(const std::span<const Mesh> others) noexcept {
  std::ranges::for_each(others.begin(), others.end(), [&](const auto& other) {
    vertices_.insert(vertices_.end(), other.vertices_.begin(), other.vertices_.end());
    elements_.insert(elements_.end(), other.elements_.begin(), other.elements_.end());
  });
}
...
void Mesh::recalculateNormals(size_t idxOffset) noexcept {
  std::ranges::for_each(vertices_.begin(), vertices_.end(), [](auto& v) { v.normal = glm::vec3(0); });
  ...
  std::ranges::for_each(vertices_.begin(), vertices_.end(), [](auto& v) {
    auto len = glm::length(v.normal);
    if (len > 0.0f) {
...
void Mesh::scale(glm::vec3 scale, glm::mat4 transform) noexcept { transformPositions(glm::scale(transform, scale)); }
...
Mesh Mesh::Cube(idx_t* idxOffset, const glm::mat4& transform) noexcept {
  u32 idx = (idxOffset != nullptr) ? *idxOffset : 0;
  ...
  for (size_t i = 0; i < faceCount; i++) {
    ...
    for (auto& vertex : face.vertices()) {
      ...
      vertex.normal = glm::normalize(glm::inverseTranspose(glm::mat3(localTransform)) * vertex.normal);
    }
  }

  if (idxOffset != nullptr) {
    *idxOffset += idx - *idxOffset;
  }
  return Mesh(faces);
}

Mesh Mesh::Plane(idx_t* idxOffset, const glm::mat4& transform) noexcept {
  size_t idx = (idxOffset != nullptr) ? *idxOffset : 0;
  constexpr Vertex::normal_type normal(0, 0, 1);

  std::vector<Vertex> vertices;
  vertices.reserve(4);
  vertices.push_back(Vertex{.position = {0, 0, 0}, .normal = normal});
  vertices.push_back(Vertex{.position = {1, 0, 0}, .normal = normal});
  vertices.push_back(Vertex{.position = {1, 1, 0}, .normal = normal});
  vertices.push_back(Vertex{.position = {0, 1, 0}, .normal = normal});

  std::ranges::for_each(vertices.begin(), vertices.end(), [transform](auto& vertex) {
    vertex.position = transform * glm::vec4(vertex.position, 1);
    vertex.normal = glm::normalize(glm::inverseTranspose(glm::mat3(transform)) * vertex.normal);
  });

  std::vector<idx_t> elements = {1, 3, 0, 1, 2, 3};
  std::ranges::for_each(elements.begin(), elements.end(), [idx](auto& i) { i += idx; });
  if (idxOffset != nullptr) {
    *idxOffset += 4;
  }
...
void Mesh::color(const Vertex::color_type color) noexcept {
  std::ranges::for_each(vertices_.begin(), vertices_.end(), [&color](auto& vertex) { vertex.color = color; });
}
...
void displacePositions(Mesh* mesh, const glm::vec3& range) {
  ...
  for (auto& vertex : mesh->vertices()) {
    ...
    if (std::ranges::find_if(map.begin(), map.end(), [&position](auto& kvp) {
          return glm::length(kvp.first - position) < epsilon_f;
        }) == map.end()) {
      map.emplace_back(position, glm::vec3(displacement_x(bitgen), displacement_y(bitgen), displacement_z(bitgen)));
    }
  }
  std::for_each(mesh->vertices().begin(), mesh->vertices().end(), [&map](auto& vertex) {
    auto rand = std::find_if(map.begin(), map.end(),
                             [&vertex](auto& kvp) { return glm::length(kvp.first - vertex.position) < epsilon_f; });
    vertex.position = glm::translate(glm::mat4(1), rand->second) * glm::vec4(vertex.position, 1);
  });
}
...
void displaceRotation(Mesh* mesh, f32 deg, const glm::vec3& axis) {
  ...
  for (auto& vertex : mesh->vertices()) {
    ...
    if (std::ranges::find_if(map.begin(), map.end(), [&rotation](auto& kvp) {
          return glm::length(kvp.first - rotation) < epsilon_f;
        }) == map.end()) {
      map.emplace_back(rotation, glm::vec3(displacement(bitgen), displacement(bitgen), displacement(bitgen)) * axis);
    }
  }
  std::for_each(mesh->vertices().begin(), mesh->vertices().end(), [&map](auto& vertex) {
    auto rand = std::find_if(map.begin(), map.end(),
                             [&vertex](auto& kvp) { return glm::length(kvp.first - vertex.position) < epsilon_f; });
    auto rotation = glm::rotate(glm::mat4(1), glm::radians(rand->second.x), glm::vec3(1, 0, 0));
    rotation = glm::rotate(rotation, glm::radians(rand->second.y), WorldUp);
    rotation = glm::rotate(rotation, glm::radians(rand->second.z), glm::vec3(0, 0, 1));
    vertex.position = rotation * glm::vec4(vertex.position, 1.0);
  });
}

Mesh Mesh::Hexagon::Fill(const Hex* hex, f32 rad, f32 fill, Vertex::color_type color) noexcept {
  std::vector<Vertex> vertices;
  vertices.reserve(18);
  ...
  if (points.size() != 0) {
    for (size_t i = 0; i < points.size(); i++) {
      vertices.emplace_back(Vertex::position_type(center, 0).xzy(), WorldUp, color);
      vertices.emplace_back(Vertex::position_type(points.at(i % points.size()), 0).xzy(), WorldUp, color);
      vertices.emplace_back(Vertex::position_type(points.at((i + 1) % points.size()), 0).xzy(), WorldUp, color);
    }
  }
...
Mesh Mesh::Hexagon::Fill(const HexGroup* group) noexcept {
  ...
  std::ranges::for_each(hexagons.begin(), hexagons.end(), [&](const auto& hex) {
    result.merge(Fill(&hex, group->size, group->fillPercentage, group->fillColor(hex)));
  });
  return result;
}

Mesh Mesh::Hexagon::Outline(const Hex* hex, f32 radius, f32 fill, Vertex::color_type color) noexcept {
  std::vector<Vertex> vertices;
  vertices.reserve(7);
  for (auto point : hex->generatePoints(radius, fill)) {
    vertices.emplace_back(Vertex::position_type(point, 0).xzy(), WorldUp, color);
  }
...
Mesh Mesh::Hexagon::Outline(const HexGroup* group) noexcept {
  ...
  auto hexagons = group->generateHexagons();
  std::ranges::for_each(hexagons.begin(), hexagons.end(), [&](const auto& hex) {
    result.merge(Outline(&hex, group->size, group->fillPercentage, uinta::HexGroup::outlineColor(hex)));
  });
  return result;
}
...
Mesh Mesh::Environment::Tree(idx_t* idxOffset, glm::mat4 transform) noexcept {
  ...
  static auto genTrunk = [&](idx_t* idxOffset, const glm::mat4& transform) noexcept {
    auto idx = *idxOffset;
    auto cube = Cube(idxOffset);
  ...
  std::array<Mesh, 2> meshes = {genCanopy(idxOffset, transform), genTrunk(idxOffset, transform)};
  return Mesh(meshes);
}
