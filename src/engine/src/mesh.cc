...
Mesh::Mesh(const std::span<const Mesh> collection) noexcept {
  std::ranges::for_each(
      collection.begin(), collection.end(), [this](auto& mesh) {
...
void Mesh::recalculateNormals(size_t idxOffset) noexcept {
  std::ranges::for_each(vertices_.begin(), vertices_.end(),
...
Mesh Mesh::Plane(idx_t* idxOffset, const glm::mat4& transform) noexcept {
  ...
  std::ranges::for_each(
      vertices.begin(), vertices.end(), [transform](auto& vertex) {
  ...
  std::ranges::for_each(elements.begin(), elements.end(),
                        [idx](auto& i) { i += idx; });
  if (idxOffset != nullptr) {
    *idxOffset += 4;
  }
...
void displacePositions(Mesh* mesh, const glm::vec3& range) {
  ...
  for (auto& vertex : mesh->vertices()) {
    ...
    if (std::ranges::find_if(map.begin(), map.end(), [&position](auto& kvp) {
      ...
      map.emplace_back(position,
                       glm::vec3(displacement_x(bitgen), displacement_y(bitgen),
...
void displaceRotation(Mesh* mesh, f32 deg, const glm::vec3& axis) {
  ...
  for (auto& vertex : mesh->vertices()) {
    ...
    if (std::ranges::find_if(map.begin(), map.end(), [&rotation](auto& kvp) {
      ...
      map.emplace_back(rotation,
                       glm::vec3(displacement(bitgen), displacement(bitgen),
                                 displacement(bitgen)) *
                           axis);
    }
  }
...
Mesh Mesh::Hexagon::Outline(const Hex* hex, f32 radius, f32 fill,
                            Vertex::color_type color) noexcept {
  std::vector<Vertex> vertices;
  vertices.reserve(7);
...
Mesh Mesh::Hexagon::Outline(const HexGroup* group) noexcept {
  ...
  std::ranges::for_each(hexagons.begin(), hexagons.end(), [&](const auto& hex) {
    result.merge(Outline(&hex, group->size, group->fillPercentage,
                         uinta::HexGroup::outlineColor(hex)));
  });
  return result;
}
