...
GridScene::GridScene(Scene* parent, Params params) noexcept
  ...
  cameraManager->addListener<CameraEvent::ViewMatrixUpdated>([this](const auto& event) {
    view_ = event.view;
...
void GridScene::generate() noexcept {
  ...
  for (i32 i = -irad; i <= irad; i++) {
    ...
    std::array<glm::vec2, 4> points({{-frad * params_.cellWidth, if_ * params_.cellHeight},
                                     {frad * params_.cellWidth, if_ * params_.cellHeight},
                                     {if_ * params_.cellWidth, -frad * params_.cellHeight},
                                     {if_ * params_.cellWidth, frad * params_.cellHeight}});
    std::ranges::for_each(points.begin(), points.end(), [this](auto& point) { point += params_.position; });
    vertices.insert(vertices.end(), points.begin(), points.end());
  }

  VaoGuard vag(&vao_);
  VboGuard vbg(&vbo_);
  vbo_.bufferData(vertices.data(), static_cast<GLsizeiptr>(vertices.size() * sizeof(glm::vec2)), GL_STATIC_DRAW);
...
void GridScene::onDebugUi() noexcept {
  auto update = ImGui::InputFloat2("Positios##grid", glm::value_ptr(params_.position));
...
void GridScene::render(time_t /*unused*/) noexcept {
  ...
  if (flags_.isViewDirty()) {
    ...
    params_.position = glm::vec2(std::floor(cameraTarget_.x / params_.cellWidth) * params_.cellWidth,
