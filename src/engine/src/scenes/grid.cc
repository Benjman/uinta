...
GridScene::GridScene(Scene* parent, size_t size, const Camera* camera,
...
void GridScene::tick(const EngineState& state) noexcept {
...
void GridScene::onViewportSizeChange(const ViewportSizeChange& event) noexcept {
  ShaderGuard sg(&shader_);
