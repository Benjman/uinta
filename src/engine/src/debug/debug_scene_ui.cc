...
f32 UiSpacing = 0;
f32 UiWidth = 300;
f32 UiHalfWidth = static_cast<f32>(UiWidth / 2.0);
f32 UiOneThirdsWidth = static_cast<f32>(UiWidth * 1.0 / 3.0);
f32 UiTwoThirdsWidth = static_cast<f32>(UiWidth * 2.0 / 3.0);
f32 UiOneQuartersWidth = static_cast<f32>(UiWidth * 1.0 / 4.0);
f32 UiOneFifthsWidth = static_cast<f32>(UiWidth * 1.0 / 5.0);
f32 UiThreeQuartersWidth = static_cast<f32>(UiWidth * 3.0 / 4.0);
...
DebugSceneUi::DebugSceneUi(Scene* parent) noexcept
  ...
  camera_ = parent->engine()->service<CameraManager>();
  assert(camera_);

  static time_t prevRenderComplete = 0;
  auto* engine = parent->engine();
  engineUiInfo_.engine = engine;
  engine->dispatchers()->addListener<EngineEvent::RenderComplete>([](const RenderComplete& event) {
    engineUiInfo_.render.push(static_cast<f32>(event.runtime - prevRenderComplete));
    prevRenderComplete = event.runtime;
  });

  static time_t prevTickComplete = 0;
  engine->dispatchers()->addListener<EngineEvent::TickComplete>([](const TickComplete& event) {
    engineUiInfo_.tick.push(static_cast<f32>(event.runtime - prevTickComplete));
    engineUiInfo_.delta.push(static_cast<f32>(event.state->delta()));
  ...
  if (!flags_.isImGuiInitialized()) {
    ...
    style.WindowMinSize = ImVec2(UiWidth + (style.DisplayWindowPadding.x * 2) + style.ScrollbarSize, 400);
    style.WindowRounding = 8;
    style.ScrollbarRounding = 4;

    UiSpacing = style.ItemSpacing.x;
    UiOneThirdsWidth -= static_cast<f32>(UiSpacing * (1.0 / 3.0));
    UiTwoThirdsWidth -= static_cast<f32>(UiSpacing * (2.0 / 3.0));
    UiOneQuartersWidth -= static_cast<f32>(UiSpacing * (1.0 / 4.0));
    UiOneFifthsWidth -= static_cast<f32>(UiSpacing * (1.0 / 5.0));
    UiThreeQuartersWidth -= static_cast<f32>(UiSpacing * (3.0 / 4.0));

    ImGui_ImplGlfw_InitForOpenGL(static_cast<GLFWwindow*>(engine->platform()->window()->userData()), true);
  ...
  class FpsCounter : public PostRenderComponent {
   public:
    FpsCounter(Engine* /*unused*/, EngineUiInfo::FpsCounterAvg* /*unused*/) noexcept {}

    void update(time_t delta) noexcept override {
      counter++;
      runtime_ += delta;
      auto currentSecond = static_cast<size_t>(runtime_);
      if (auto casted = static_cast<size_t>(runtime_); static_cast<f32>(currentSecond) > prevSecond) {
        prevSecond = static_cast<f32>(casted);
        engineUiInfo_.fps.push(static_cast<f32>(counter));
        counter = 0;
      }
    }
...
void DebugSceneUi::render(time_t /*unused*/) noexcept {
  ...
  if (camera_ != nullptr) {
    RenderCameraUi(camera_, viewport_);
  }
