...
DebugSceneUi::DebugSceneUi(Scene* parent) noexcept
  ...
  engine->dispatchers()->addListener<EngineEvent::RenderComplete>(
      [](const RenderComplete& event) {
        engineUiInfo_.render.push(
            static_cast<f32>(event.runtime - prevRenderComplete));
  ...
  engine->dispatchers()->addListener<EngineEvent::TickComplete>(
      [](const TickComplete& event) {
        engineUiInfo_.tick.push(
            static_cast<f32>(event.runtime - prevTickComplete));
  ...
  if (!flags_.isImGuiInitialized()) {
    ...
    style.WindowMinSize = ImVec2(
        UiWidth + (style.DisplayWindowPadding.x * 2) + style.ScrollbarSize,
        400);
  ...
  class FpsCounter : public PostRenderComponent {
   public:
    FpsCounter(Engine* /*unused*/,
               EngineUiInfo::FpsCounterAvg* /*unused*/) noexcept {}
    ...
    void update(time_t delta) noexcept override {
      ...
      if (auto casted = static_cast<size_t>(runtime_);
          static_cast<f32>(currentSecond) > prevSecond) {
