...
Scene::Scene(Engine* engine, SceneLayer layer, SceneState state) noexcept : components_(this), engine_(engine) {
  flags_.layer(layer);
  flags_.state(state);
}
...
void Scene::layer(SceneLayer layer) noexcept {
  ...
  flags_.layer(layer);
  dispatchers_.dispatch<SceneEvent::LayerChange>(SceneLayerChangeEvent(this, prevLayer, layer));
}
...
void Scene::state(SceneState state) noexcept {
  ...
  flags_.state(state);
  dispatchers_.dispatch<SceneEvent::StateChange>(SceneStateChangeEvent(this, prevState, state));
}

Status Scene::onSceneLayerChange(const SceneLayerChangeEvent& event) noexcept {
  LOG(INFO) << absl::StrFormat("%s layer changed to %s", event.scene->name(), to_string(event.scene->layer()));
  return status_;
}

Status Scene::onSceneStateChange(const SceneStateChangeEvent& event) noexcept {
  if (this != event.scene->parent_ || this == event.scene) {
    return InvalidArgumentError("Mismanged attempt to handle child scene state change.");
  }
  if (event.scene->state() == SceneState::Complete) {
    auto itr = std::ranges::find_if(children_.begin(), children_.end(),
                                    [&event](const auto& scene) { return scene.get() == event.scene; });
    if (itr == children_.end()) {
      return InternalError("Failed to locate scene within children collection.");
    }
    return removeScene(itr->get());
  }
  LOG(INFO) << absl::StrFormat("%s state changed to %s", event.scene->name(), to_string(event.scene->state()));
  return status_;
}

Status Scene::removeScene(Scene* scene) noexcept {
  if (this == scene) {
    return InvalidArgumentError("Attempt to remove self from scene stack detected.");
  }
  if (children_.contains(scene)) {
    ...
    constexpr bool ShouldCallParent = false;
    if constexpr (ShouldCallParent) {
      return parent_->removeScene(scene);
    }
    std::string errorMsg = "Attempting to remove %s failed: Could not locate in scene stack.";
    auto childName = name();
    Scene* parent = parent_;
    while (parent != nullptr) {
      errorMsg = absl::StrCat(errorMsg, "\r\t", childName, " is a child of ", parent->name());
