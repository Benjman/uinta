...
Scene::Scene(Engine* engine, SceneLayer layer, SceneState state) noexcept
    : components_(this), engine_(engine) {
  flags_.layer(layer);
  flags_.state(state);
}

Scene::Scene(Scene* parent, SceneLayer layer, SceneState state) noexcept
    : Scene(parent->engine(), layer, state) {
