...
void Engine::updateRenderOrder() noexcept {
  struct Comparator final {
    bool operator()(const Scene* a, const Scene* b) const noexcept { return a->layer() > b->layer(); }
  };
  ...
  if (!sceneQueue_.empty()) {
    ...
    queue.push(front);
    std::for_each(front->children().begin(), front->children().end(), [&queue](auto& scene) {
...
void Engine::registerSceneListeners(Scene* scene) noexcept {
  ...
  scene->dispatchers()->template addListener<SceneEvent::SceneAdded>([this](const auto&) { updateRenderOrder(); });
  scene->dispatchers()->template addListener<SceneEvent::SceneRemoved>([this](const auto&) { updateRenderOrder(); });
  scene->dispatchers()->template addListener<SceneEvent::LayerChange>([this](const auto&) { updateRenderOrder(); });
