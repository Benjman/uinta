...
Engine::Engine(Params params) noexcept
    : localization_(resolveLocale(params.args, params.locale)),
      ...
      shader_({{GL_VERTEX_SHADER, "shader.vs.glsl"}, {GL_FRAGMENT_SHADER, "shader.fs.glsl"}}, params.gl),
  ...
  dispatchers_.addListener<EngineEvent::ViewportSizeChange>([&](const auto& event) {
    ...
    uProjection_ = glm::perspective(fov, event.aspect(), nearPlane, farPlane);
  });
...

  {
    VboGuard vbg(&vbo_);
    VaoGuard vag(&vao_);
    vbo_.bufferData(vertices.data(), vertices.size() * sizeof(GLfloat), GL_STATIC_DRAW);
