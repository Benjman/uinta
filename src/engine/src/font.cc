...
Font::Font(const std::string& path, i32 bmapWidth, i32 bmapHeight,
           f32 bmapLineSize, const OpenGLApi* gl) noexcept
  ...
  std::vector<u8> bitmap(
      static_cast<u64>(texture_.width() * texture_.height() * Channels));

  if (stbtt_PackBegin(&ctx, bitmap.data(), texture_.width(), texture_.height(),
                      Stride, Padding, AllocContext) == 0) {
    status_ = InternalError("stbtt_PackBegin failure.");
    return;
  }

  if (stbtt_PackFontRange(&ctx, fontData.data(), FontIndex, lineSize_,
                          FirstChar, CharCount, chars_.data()) == 0) {
  ...
  LOG(INFO) << absl::StrFormat("Loaded font: %s",
                               resolvedPath.value().string());
}
...
StatusOr<stbtt_aligned_quad> Font::quad(i8 c, f32* x, f32* y,
  ...
  stbtt_GetPackedQuad(chars_.data(), texture_.width(), texture_.height(),
                      static_cast<i32>(c - FirstChar), x, y, &quad, AlignToInt);
  quad.x0 *= scale;
  quad.x1 *= scale;
  quad.y0 *= scale;
  quad.y1 *= scale;
...
void TextMeshBuilder::TextMeshGenerator::reserve(
  ...
  auto count = 0;
  std::ranges::for_each(words.begin(), words.end(), [&count](const auto& word) {
    count += word.quads.size();
  });
  vertices_.reserve(
      static_cast<i32>(count * ElementsPerTextVertex * VerticesPerTextQuad));
}
