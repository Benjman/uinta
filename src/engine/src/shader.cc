...
struct ShaderValidator final {
  ShaderValidator(GLuint id, const OpenGLApi* gl) noexcept {
    ...
    if (success == GL_ZERO) {
      ...
      std::array<GLchar, kInfoLen> info;
      GLsizei logSize;
      gl->getProgramInfoLog(id, kInfoLen, &logSize, info.data());
      LOG(FATAL) << std::string(info.data(), logSize);
      return;
    }
  }
  ...
  [[nodiscard]] auto message() const noexcept { return status_.message(); }
...
struct ShaderStageValidator final {
  ShaderStageValidator(GLuint shaderId, std::string_view path, const OpenGLApi* gl) noexcept {
    ...
    if (isCompiled == GL_ZERO) {
      ...
      std::array<GLchar, kInfoLen> info;
      GLsizei logSize;
      gl->getShaderInfoLog(shaderId, kInfoLen, &logSize, info.data());
      gl->deleteShader(shaderId);
      status_ = InternalError(absl::StrFormat("%s: %s", path, std::string(info.data(), logSize)));
    }
  }
...
Shader::Shader(const std::unordered_map<GLenum, std::string>& stagePaths, const OpenGLApi* gl) noexcept
    : id_(gl->createProgram()), gl_(gl) {
  for (const auto& pair : stagePaths) {
    auto resolvedPath = FindFile(pair.second);
    if (!resolvedPath.has_value()) {
      LOG(FATAL) << "Shader file not found: " << pair.second;
      return;
    }

    std::string src;
    if (auto status = ReadFile(resolvedPath.value(), src); !status.ok()) {
      LOG(FATAL) << status.message();
      return;
    }

    LOG(INFO) << "Compiling shader: " << resolvedPath.value().string();
