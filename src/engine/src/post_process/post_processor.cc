...
PostProcessor::PostProcessor(Engine* engine) noexcept
  ...
  // viewport manager.
  if (auto* platform = engine_->platform(); platform != nullptr && platform->window() != nullptr) {
  ...
  if (auto* vp = engine_->service<ViewportManager>()) {
    ...
    vp->addListener<ViewportEvent::ProjectionMatrixUpdated>([this](const ProjectionMatrixUpdateEvent& event) {
  ...
  if (auto* cm = engine_->service<CameraManager>(); cm != nullptr) {
    cm->addListener<CameraEvent::ViewMatrixUpdated>([&](const ViewMatrixUpdateEvent& e) { view_ = e.view; });
  }

  engine_->dispatchers()->addListener<EngineEvent::ViewportSizeChange>(
      [this](const ViewportSizeChange& e) { resize(std::max<u32>(1, e.width), std::max<u32>(1, e.height)); });

  engine_->dispatchers()->addListener<EngineEvent::RenderLayerChange>([this](const RenderLayerChange& e) {
...
void PostProcessor::removePass(const PostProcessPass* pass) noexcept {
  auto it = std::ranges::find_if(passes_.begin(), passes_.end(),
                                 [pass](const std::unique_ptr<PostProcessPass>& p) { return p.get() == pass; });
  if (it != passes_.end()) {
    passes_.erase(it);
  }
}
...
void PostProcessor::allocateTargets() noexcept {
  sceneTarget_ = std::make_unique<RenderTarget>(gl_, width_, height_, RenderTargetFormats::Hdr(),
                                                RenderTargetFormats::Depth24(), "post_process::sceneTarget");
  for (u32 i = 0; i < pingPong_.size(); ++i) {
    pingPong_[i] = std::make_unique<RenderTarget>(gl_, width_, height_, RenderTargetFormats::Hdr(), std::nullopt,
...
void PostProcessor::beginSimulation() noexcept {
  ...
  gl_->viewport(0, 0, static_cast<GLsizei>(width_), static_cast<GLsizei>(height_));
