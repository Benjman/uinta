...
void BlitHelper::draw(const Texture* source, const Fbo* destFbo, u32 width, u32 height) const noexcept {
  // Bind destination (nullptr = default framebuffer).
  gl_->bindFramebuffer(GL_FRAMEBUFFER, (destFbo != nullptr) ? destFbo->id() : 0);
  gl_->viewport(0, 0, static_cast<GLsizei>(width), static_cast<GLsizei>(height));
