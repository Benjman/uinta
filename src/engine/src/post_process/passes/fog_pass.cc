...
void FogPass::onDebugUi() noexcept {
  ...
  float d = density_;
  if (ImGui::SliderFloat("Density", &d, 0.0f, 0.2f, "%.4f", ImGuiSliderFlags_Logarithmic)) {
    density_ = d;
  }
  ...
  ImGui::TextWrapped(
      "Depth-based exponential fog. Set density to a small value (try 0.01). 'Far Fade Start' is the fraction of the "
      "far plane at which fog smoothly saturates to its color (1.0 disables the fade).");
}
