...
Status ColorGradingPass::setLut(const std::string& resourcePath) noexcept {
  ...
  auto tex = std::make_unique<Texture>(GL_TEXTURE_2D, 0, 0, 0, GL_LINEAR, GL_CLAMP_TO_EDGE, gl_);
  if (auto status = tex->fromFile(resourcePath); !status.ok()) {
    LOG(ERROR) << "ColorGradingPass: failed to load LUT '" << resourcePath << "': " << status;
    return status;
  }
  ...
  if (h <= 0 || w != h * h) {
    return InvalidArgumentError(
        absl::StrFormat("LUT '%s' has invalid dimensions %dx%d; expected (N*N) x N (i.e. width == height * height).",
                        resourcePath, w, h));
  }
  lut_ = std::move(tex);
  lutSize_ = static_cast<u32>(h);
  lutPath_ = resourcePath;
  LOG(INFO) << "ColorGradingPass: loaded LUT '" << resourcePath << "' (size " << lutSize_ << ")";
  return OkStatus();
}
