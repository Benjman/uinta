...
void OutlinePass::execute(const PassContext& ctx) noexcept {
  if (ctx.input == nullptr || ctx.sceneTarget == nullptr || ctx.sceneTarget->depth() == nullptr) {
    return;
  }
  ...
  uTexelSize_ = glm::vec2(1.0f / static_cast<f32>(std::max<u32>(1, ctx.width)),
...
void OutlinePass::onDebugUi() noexcept {
  ...
  ImGui::TextWrapped(
      "Cel/toon outline from depth and reconstructed normals. Increase depth sensitivity for stronger silhouettes; "
      "increase normal sensitivity for inner creases. 'Far Fade Start' is the fraction of the far plane at which "
      "outlines begin fading to zero (1.0 disables the fade).");
}
