...
BloomPass::BloomPass(const OpenGLApi* gl) noexcept
    : gl_(gl),
      downsampleShader_(
          {
              ...
              {GL_FRAGMENT_SHADER, "shader/post_process/bloom_downsample.fs.glsl"},
          },
          gl),
      ...
      upsampleShader_(
          {
              {GL_VERTEX_SHADER, "shader/post_process/fullscreen.vs.glsl"},
              {GL_FRAGMENT_SHADER, "shader/post_process/bloom_upsample.fs.glsl"},
          },
          gl),
      ...
      compositeShader_(
          {
              {GL_VERTEX_SHADER, "shader/post_process/fullscreen.vs.glsl"},
              {GL_FRAGMENT_SHADER, "shader/post_process/bloom_composite.fs.glsl"},
          },
          gl),
...
void BloomPass::execute(const PassContext& ctx) noexcept {
  if (ctx.input == nullptr || ctx.pool == nullptr || ctx.fullscreenVao == nullptr) {
    return;
  }
  if (intensity_ <= 0.0f) {
  ...
  // --- Downsample chain -------------------------------------------------
  {
    ...
    for (u32 mip = 0; mip < levels; ++mip) {
      ...
      if (mip > 0) {
        gl_->texParameteri(srcTex->target(), GL_TEXTURE_BASE_LEVEL, static_cast<GLint>(mip - 1));
        gl_->texParameteri(srcTex->target(), GL_TEXTURE_MAX_LEVEL, static_cast<GLint>(mip - 1));
      }

      const Fbo* dst = mipTarget->fbo(mip);
      gl_->bindFramebuffer(GL_FRAMEBUFFER, (dst != nullptr) ? dst->id() : 0);
      gl_->viewport(0, 0, static_cast<GLsizei>(mipTarget->width(mip)), static_cast<GLsizei>(mipTarget->height(mip)));

      dsTexelSize_ = glm::vec2(1.0f / static_cast<f32>(srcW), 1.0f / static_cast<f32>(srcH));
      ...
      if (mip > 0) {
        ...
        gl_->texParameteri(srcTex->target(), GL_TEXTURE_MAX_LEVEL, static_cast<GLint>(levels - 1));
      }
    }
  ...
  // --- Upsample chain (additive) ---------------------------------------
  {
    ...
    for (u32 mip = levels - 1; mip > 0; --mip) {
      gl_->texParameteri(chain->target(), GL_TEXTURE_BASE_LEVEL, static_cast<GLint>(mip));
      gl_->texParameteri(chain->target(), GL_TEXTURE_MAX_LEVEL, static_cast<GLint>(mip));
    ...
    gl_->texParameteri(chain->target(), GL_TEXTURE_MAX_LEVEL, static_cast<GLint>(levels - 1));
  ...
  // --- Composite into output ------------------------------------------
  {
    ...
    gl_->bindFramebuffer(GL_FRAMEBUFFER, (ctx.output != nullptr) ? ctx.output->id() : 0);
    gl_->viewport(0, 0, static_cast<GLsizei>(ctx.width), static_cast<GLsizei>(ctx.height));
...
void BloomPass::onDebugUi() noexcept {
  ...
  ImGui::TextWrapped(
      "Dual-filter bloom (Jimenez 2014). Runs a %u-level downsample/upsample chain and composites onto the scene "
      "additively. Set intensity to 0 to bypass.",
      mipCount_);
}
