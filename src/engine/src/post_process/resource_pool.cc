...
bool SameFormat(const TextureFormat& a, const TextureFormat& b) noexcept {
  return a.internalFormat == b.internalFormat && a.pixelFormat == b.pixelFormat && a.pixelType == b.pixelType &&
...
RenderTarget* ResourcePool::acquire(const ResourceDesc& desc) noexcept {
  ...
  Entry entry;
  entry.desc = desc;
  entry.target = std::make_unique<RenderTarget>(gl_, desc.width, desc.height, desc.color, desc.depth, desc.debugName);
  entry.inUse = true;
