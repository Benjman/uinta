...
RenderTarget::RenderTarget(const OpenGLApi* gl, u32 width, u32 height, TextureFormat colorFormat,
                           std::optional<TextureFormat> depthFormat, std::string debugName) noexcept
    : gl_(gl),
      width_(width),
      height_(height),
...
void RenderTarget::allocate() noexcept {
  ...
  color_ = std::make_unique<Texture>(GL_TEXTURE_2D, static_cast<i32>(width_), static_cast<i32>(height_), colorFormat_,
                                     GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, gl_);

  // Allocate depth texture if requested.
  if (depthFormat_.has_value()) {
    depth_ = std::make_unique<Texture>(GL_TEXTURE_2D, static_cast<i32>(width_), static_cast<i32>(height_),
  ...
  for (u32 mip = 0; mip < levels; ++mip) {
    auto fbo = std::make_unique<Fbo>(gl_);
    {
      ...
      fbo->setDrawBuffers();
      if (!fbo->isComplete()) {
        LOG(ERROR) << "RenderTarget '" << debugName_ << "' FBO incomplete at mip " << mip << " (status=0x" << std::hex
                   << fbo->checkStatus() << std::dec << ")";
      }
    }
...
u32 RenderTarget::width(u32 mip) const noexcept { return std::max<u32>(1, width_ >> mip); }

u32 RenderTarget::height(u32 mip) const noexcept { return std::max<u32>(1, height_ >> mip); }
