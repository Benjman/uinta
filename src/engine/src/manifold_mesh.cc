...
static_assert(sizeof(idx_t) >= sizeof(uint32_t), "uinta::idx_t must be at least 32-bit to hold manifold indices");
static_assert(std::is_unsigned_v<idx_t>, "uinta::idx_t is expected to be an unsigned integer type");
...
Mesh ToMesh(const manifold::Manifold& m, double minSharpAngle) noexcept {
  ...
  const manifold::Manifold withNormals = m.CalculateNormals(NormalIdx, minSharpAngle);
  ...
  for (size_t v = 0; v < numVerts; ++v) {
    ...
    Vertex::position_type position{mesh.vertProperties[offset + PositionOffset + 0],
                                   mesh.vertProperties[offset + PositionOffset + 1],
                                   mesh.vertProperties[offset + PositionOffset + 2]};

    Vertex::normal_type normal{};
    if (hasNormals) {
      normal = Vertex::normal_type{mesh.vertProperties[offset + NormalOffset + 0],
