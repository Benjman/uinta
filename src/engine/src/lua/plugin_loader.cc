...
StatusOr<std::vector<PluginInfo>> PluginLoader::discoverPlugins(const std::vector<fs::path>& searchPaths) const {
  std::vector<PluginInfo> result;
  std::unordered_set<std::string> seenIds;

  for (const auto& searchPath : searchPaths) {
    if (!fs::exists(searchPath)) {
      LOG(WARNING) << "Plugin search path does not exist: " << searchPath.string();
      continue;
    }

    if (!fs::is_directory(searchPath)) {
      LOG(WARNING) << "Plugin search path is not a directory: " << searchPath.string();
      continue;
    }
    ...
    for (const auto& entry : fs::directory_iterator(searchPath)) {
      ...
      if (seenIds.contains(id)) {
        LOG(INFO) << "Skipping duplicate plugin '" << id << "' found in " << searchPath.string();
        continue;
      }
...
StatusOr<PluginInfo> PluginLoader::loadPluginInfo(const std::string& pluginPath) const {
  fs::path pluginDir(pluginPath);
  fs::path pluginYaml = pluginDir / "plugin.yaml";

  if (!fs::exists(pluginYaml)) {
    return NotFoundError(absl::StrFormat("Plugin metadata not found: %s", pluginYaml.string()));
  }
  ...
  try {
    ...
    if (root["id"]) {
      ...
      return InvalidArgumentError(absl::StrFormat("Plugin missing 'id' field: %s", pluginPath));
    }
    ...
    if (root["api_version"]) {
      info.apiVersion = ApiVersion::parse(root["api_version"].as<std::string>());
    }
    ...
    return InternalError(absl::StrFormat("Failed to parse plugin.yaml: %s", e.what()));
  }
}

StatusOr<std::vector<std::string>> PluginLoader::resolveLoadOrder(const std::vector<PluginInfo>& plugins) const {
  ...
  for (const auto& plugin : plugins) {
    for (const auto& dep : plugin.dependencies) {
      if (!inDegree.contains(dep)) {
        return InvalidArgumentError(absl::StrFormat("Plugin '%s' depends on unknown plugin '%s'", plugin.id, dep));
      }
