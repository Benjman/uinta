...
struct LuaEventSystem::Impl {
  ...
  void on(const std::string& eventName, sol::function callback) { listeners[eventName].push_back(std::move(callback)); }

  void off(const std::string& eventName, const sol::function& callback) {
    auto it = listeners.find(eventName);
    if (it != listeners.end()) {
      auto& callbacks = it->second;
      std::erase_if(callbacks, [&callback](const sol::function& fn) { return fn == callback; });
    }
  }
    ...
    if (it != listeners.end()) {
      for (auto& callback : it->second) {
        ...
        if (!result.valid()) {
          ...
          LOG(ERROR) << "[Lua Event] Error in event '" << eventName << "': " << err.what();
        }
      }
    }
  }
...
struct LuaRuntime::Impl {
  ...
  static size_t stageIndex(EngineStage stage) { return static_cast<size_t>(stage); }
...
LuaRuntime::LuaRuntime(Engine* engine, Flags flags) : flags_(std::move(flags)), impl_(std::make_unique<Impl>()) {
  impl_->engine = engine;
...
Status LuaRuntime::initialize() {
  ...
  impl_->lua->open_libraries(sol::lib::base, sol::lib::package, sol::lib::coroutine, sol::lib::string, sol::lib::table,
  ...
  LOG(INFO) << "Lua runtime initialized (API version " << CURRENT_LUA_API_VERSION.toString() << ")";
...
Status LuaRuntime::loadPlugins(const std::vector<fs::path>& searchPaths) {
  ...
  for (const auto& pluginId : *orderResult) {
    ...
    if (!CURRENT_LUA_API_VERSION.isCompatible(plugin.apiVersion)) {
      handleLuaError("Plugin " + plugin.id,
                     absl::StrFormat("Requires API version %s, engine provides %s", plugin.apiVersion.toString(),
    ...
    if (!status.ok()) {
      handleLuaError("Loading plugin " + plugin.id, std::string(status.message()));
    }
  }
...
Status LuaRuntime::executeScript(const std::string& script, const std::string& chunkName) {
  if (!impl_->initialized) {
    auto status = initialize();
    if (!status.ok()) {
      return status;
  ...
  try {
    ...
    if (!result.valid()) {
      sol::error err = result;
      return InternalError(absl::StrFormat("Lua error in %s: %s", chunkName, err.what()));
    }
    return OkStatus();
  } catch (const sol::error& e) {
    return InternalError(absl::StrFormat("Lua exception in %s: %s", chunkName, e.what()));
  }
}
...
const HotReloadManager& LuaRuntime::hotReload() const noexcept { return impl_->hotReload; }

PluginRegistry& LuaRuntime::plugins() noexcept { return impl_->registry; }

const PluginRegistry& LuaRuntime::plugins() const noexcept { return impl_->registry; }

LuaEventSystem& LuaRuntime::events() noexcept { return impl_->events; }

const LuaEventSystem& LuaRuntime::events() const noexcept { return impl_->events; }
...
void LuaRuntime::update() {
  ...
  for (const auto& pluginId : pending) {
    ...
    if (!status.ok()) {
      LOG(ERROR) << "Failed to reload plugin '" << pluginId << "': " << status.message();
    }
  }
}
...
void LuaRuntime::registerEventBindings() {
  ...
  events["off"] = [&eventImpl](const std::string& name, const sol::function& callback) {
    eventImpl.off(name, callback);
  };

  events["emit"] = [&eventImpl](const std::string& name, const sol::object& data) { eventImpl.emit(name, data); };
}
...
void LuaRuntime::registerConfigBindings() {
  ...
  if (flags_.configReadEnabled()) {
    configTable["getString"] = [config](const std::string& key) -> sol::optional<std::string> {
    ...
    configTable["getInt"] = [config](const std::string& key) -> sol::optional<i32> {
    ...
    configTable["getFloat"] = [config](const std::string& key) -> sol::optional<f32> {
    ...
    configTable["getBoolean"] = [config](const std::string& key) -> sol::optional<bool> {
  ...
  if (flags_.configWriteEnabled()) {
    configTable["setString"] = [config](const std::string& key, const std::string& value) -> bool {
      return config->setString(key, value).ok();
    };

    configTable["setInt"] = [config](const std::string& key, i32 value) -> bool {
      return config->setInt(key, value).ok();
    };

    configTable["setFloat"] = [config](const std::string& key, f32 value) -> bool {
      return config->setFloat(key, value).ok();
    };

    configTable["setBoolean"] = [config](const std::string& key, bool value) -> bool {
...
Status LuaRuntime::loadPluginScript(const PluginInfo& plugin) {
  ...
  impl_->currentPluginId.clear();
  LOG(INFO) << "Loaded plugin: " << plugin.name << " (" << plugin.id << " v" << plugin.version << ")";
...
// NOLINTNEXTLINE
void LuaRuntime::handleLuaError(const std::string& context, const std::string& error) {
#ifdef UINTA_DEBUG
  ...
  if (!impl_->currentPluginId.empty()) {
    ...
    LOG(WARNING) << "Plugin '" << impl_->currentPluginId << "' has been disabled due to error";
  }
#endif
}
...
void LuaRuntime::registerStageBindings() {
  ...
  auto registerStage = [this, &uinta](const char* onName, const char* offName, EngineStage stage) {
    uinta[onName] = [this, stage](sol::function callback) { impl_->addStageCallback(stage, std::move(callback)); };
    uinta[offName] = [this, stage](const sol::function& callback) { impl_->removeStageCallback(stage, callback); };
  };
...
void LuaRuntime::dispatchStage(EngineStage stage, f32 deltaTime) {
  ...
  auto& callbacks = impl_->stageCallbacks[uinta::LuaRuntime::Impl::stageIndex(stage)];
