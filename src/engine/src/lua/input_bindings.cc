...
void InputBindings::registerInputFunctions(sol::state& lua) {
  ...
  input["onKey"] = [this](u32 token, sol::function callback) {
    ...
    auto handle = inputSystem->subscribeKey(token, [callback](Key key, Action action, Mod mods) {
      callback(static_cast<u32>(key), static_cast<u32>(action), static_cast<u32>(mods));
    });
  ...
  input["onMouseButton"] = [this](u32 token, sol::function callback) {
    ...
    auto handle = inputSystem->subscribeMouse(token, [callback](MouseBtn button, Action action, Mod mods) {
      callback(static_cast<u32>(button), static_cast<u32>(action), static_cast<u32>(mods));
    });
  ...
  input["onMouseMove"] = [this](sol::function callback) {
    ...
    auto handle = inputSystem->subscribeMouseMove([callback](f32 x, f32 y, f32 dx, f32 dy) { callback(x, y, dx, dy); });
  ...
  input["onMouseScroll"] = [this](sol::function callback) {
    ...
    auto handle = inputSystem->subscribeMouseScroll([callback](f32 dx, f32 dy) { callback(dx, dy); });
