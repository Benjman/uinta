...
void InputBindings::onBeforeReload() {
  ...
  assert(inputSystem != nullptr && "InputSystem must be registered as a service before using input bindings");
...
void InputBindings::registerInputFunctions(sol::state& lua) {
  ...
  input["onKey"] = [this](u32 token, sol::function callback) {
    ...
    auto handle = inputSystem->subscribeKey(token, [callback](const KeyInfo& info) {
      callback(static_cast<u32>(info.key), static_cast<u32>(info.action), static_cast<u32>(info.mods));
    });
  ...
  input["onMouseButton"] = [this](u32 token, sol::function callback) {
    ...
    auto handle = inputSystem->subscribeMouse(token, [callback](const MouseButtonInfo& info) {
      callback(static_cast<u32>(info.button), static_cast<u32>(info.action), static_cast<u32>(info.mods));
    });
  ...
  input["onMouseMove"] = [this](sol::function callback) {
    ...
    auto handle = inputSystem->subscribeMouseMove(
        [callback](const MouseMoveInfo& info) { callback(info.x, info.y, info.dx, info.dy); });
  ...
  input["onMouseScroll"] = [this](sol::function callback) {
    ...
    auto handle =
        inputSystem->subscribeMouseScroll([callback](const MouseScrollInfo& info) { callback(info.dx, info.dy); });
  ...
  input["off"] = [this](SubscriptionHandle handle) {
    ...
    auto it = std::ranges::find_if(activeHandles_.begin(), activeHandles_.end(),
