...
Status HotReloadManager::watchDirectory(const std::string& directory) {
  ...
  int wd = inotify_add_watch(impl_->inotifyFd, canonicalPath.c_str(), IN_MODIFY | IN_CREATE | IN_DELETE | IN_MOVED_TO);
  if (wd < 0) {
    return InternalError("Failed to add inotify watch: " + std::string(strerror(errno)));
  }
  ...
  for (const auto& entry : fs::recursive_directory_iterator(canonicalPath)) {
    if (entry.is_directory()) {
      ...
      int subWd = inotify_add_watch(impl_->inotifyFd, subdir.c_str(), IN_MODIFY | IN_CREATE | IN_DELETE | IN_MOVED_TO);
      if (subWd >= 0) {
...
std::vector<std::string> HotReloadManager::getPendingReloads() {
  std::lock_guard lock(mutex_);
  return std::vector<std::string>{pendingReloads_.begin(), pendingReloads_.end()};
}
...
void HotReloadManager::registerPluginFile(const std::string& filePath, const std::string& pluginId) {
  std::lock_guard lock(mutex_);
...
void HotReloadManager::unregisterPluginFiles(const std::string& pluginId) {
  std::lock_guard lock(mutex_);
  std::erase_if(fileToPlugin_, [&pluginId](const auto& pair) { return pair.second == pluginId; });
}
  ...
  if (it != fileToPlugin_.end()) {
    ...
    for (const auto& [file, plugin] : fileToPlugin_) {
      ...
      if (canonical.starts_with(pluginDir.string())) {
        ...
        LOG(INFO) << "File changed in plugin directory, scheduling reload: " << plugin;
        break;
      }
    }
  }
