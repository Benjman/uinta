...
StatusOr<std::vector<Mesh>> loadFbx(const std::string_view path) noexcept {
  ...
  const auto* scene = importer.ReadFile(resolvedPath.value().c_str(),
                                        aiProcess_Triangulate |
                                            aiProcess_JoinIdenticalVertices |
                                            aiProcess_SortByPType);
  if (scene == nullptr) {
    return InternalError(absl::StrFormat("Failed to parse %s: %s",
...
Mesh processMesh(aiMesh* mesh, const aiScene* /*unused*/) noexcept {
  ...
  for (size_t i = 0; i < mesh->mNumVertices; i++) {
    ...
    vertices.push_back(
        Vertex{.position = {v.x, v.y, v.z}, .normal = {n.x, n.y, n.z}});
  }
