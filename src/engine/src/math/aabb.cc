...
glm::vec3 to_vec3(const glm::vec2& point) noexcept { return glm::vec3{point, 0.0}; }

template <typename T>
inline void init(AABB* a, const std::span<T> points) noexcept {
  static_assert(std::is_same_v<T, glm::vec3> || std::is_same_v<T, glm::vec2>, "Must be glm::vec2 or glm::vec3");
...
AABB::AABB(const std::span<f32> points, size_t components) noexcept {
  assert((components == 2 || components == 3) && "Component count invalid");
  assert(!points.empty() && "data cannot be empty");

  min = glm::vec3(std::numeric_limits<f32>::max());
  max = glm::vec3(std::numeric_limits<f32>::lowest());

  for (size_t i = 0; i < points.size(); i += components) {
    glm::vec3 p = (components == 2) ? glm::vec3(points[i], points[i + 1], 0.0f)
