...
InputSystem::InputSystem(Engine* engine) noexcept : input_(engine->input()) {
  // Initialize previous state arrays to unknown
  prevKeysDown_.fill(_::InputUnknown);
  prevMouseDown_.fill(_::InputUnknown);
}

SubscriptionHandle InputSystem::subscribeKey(u32 token, KeyCallback callback) noexcept {
  if (!isValidKeyToken(token)) {
    LOG(FATAL) << "Invalid key subscription token: must contain both a key code and an action";
  }

  auto handle = generateHandle();
  LOG(INFO) << "Subscribed to key input: token=" << std::hex << token << std::dec << ", handle=" << handle.id;
  keySubscriptions_[token].push_back({.handle = handle, .callback = std::move(callback)});
  return handle;
}

SubscriptionHandle InputSystem::subscribeMouse(u32 token, MouseButtonCallback callback) noexcept {
  if (!isValidMouseButtonToken(token)) {
    LOG(FATAL) << "Invalid mouse button subscription token: must contain an action";
  }

  auto handle = generateHandle();
  LOG(INFO) << "Subscribed to mouse button input: token=" << std::hex << token << std::dec << ", handle=" << handle.id;
  mouseSubscriptions_[token].push_back({.handle = handle, .callback = std::move(callback)});
  return handle;
}

SubscriptionHandle InputSystem::subscribeMouseMove(MouseMoveCallback callback) noexcept {
  auto handle = generateHandle();
  LOG(INFO) << "Subscribed to mouse movement, handle=" << handle.id;
  mouseMoveSubscriptions_.push_back({.handle = handle, .callback = std::move(callback)});
  return handle;
}

SubscriptionHandle InputSystem::subscribeMouseScroll(MouseScrollCallback callback) noexcept {
  auto handle = generateHandle();
  LOG(INFO) << "Subscribed to mouse scroll, handle=" << handle.id;
  mouseScrollSubscriptions_.push_back({.handle = handle, .callback = std::move(callback)});
  return handle;
}

void InputSystem::unsubscribe(SubscriptionHandle handle) noexcept {
  LOG(INFO) << "Unsubscribed from input event, handle=" << handle.id;

  // Search key subscriptions
  for (auto& [token, subscriptions] : keySubscriptions_) {
    auto it = std::ranges::find_if(subscriptions.begin(), subscriptions.end(),
  ...
  for (auto& [token, subscriptions] : mouseSubscriptions_) {
    auto it = std::ranges::find_if(subscriptions.begin(), subscriptions.end(),
                                   [&handle](const MouseButtonSubscription& sub) { return sub.handle == handle; });
    if (it != subscriptions.end()) {
      subscriptions.erase(it);
      if (subscriptions.empty()) {
  ...
  auto moveIt = std::ranges::find_if(mouseMoveSubscriptions_.begin(), mouseMoveSubscriptions_.end(),
                                     [&handle](const MouseMoveSubscription& sub) { return sub.handle == handle; });
  ...
  auto scrollIt = std::ranges::find_if(mouseScrollSubscriptions_.begin(), mouseScrollSubscriptions_.end(),
                                       [&handle](const MouseScrollSubscription& sub) { return sub.handle == handle; });
...
void InputSystem::update(time_t /*unused*/) noexcept {
...
SubscriptionHandle InputSystem::generateHandle() noexcept { return SubscriptionHandle{nextHandleId_++}; }

bool InputSystem::matchesAction(Action currentAction, Action subscribedActions) const noexcept {
  return (static_cast<u32>(currentAction) & static_cast<u32>(subscribedActions)) != 0;
}

bool InputSystem::matchesModifiers(Mod currentMods, Mod subscribedMods) const noexcept {
