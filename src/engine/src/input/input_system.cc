...
SubscriptionHandle InputSystem::subscribeKey(u32 token,
  ...
  keySubscriptions_[token].push_back(
...
SubscriptionHandle InputSystem::subscribeMouse(
  ...
  auto handle = generateHandle();
  mouseSubscriptions_[token].push_back(
      {.handle = handle, .callback = std::move(callback)});
  return handle;
}
...
SubscriptionHandle InputSystem::subscribeMouseMove(
  ...
  auto handle = generateHandle();
  mouseMoveSubscriptions_.push_back(
      {.handle = handle, .callback = std::move(callback)});
  return handle;
}
...
SubscriptionHandle InputSystem::subscribeMouseScroll(
  ...
  auto handle = generateHandle();
  mouseScrollSubscriptions_.push_back(
      {.handle = handle, .callback = std::move(callback)});
  return handle;
}
...
void InputSystem::unsubscribe(SubscriptionHandle handle) noexcept {
  ...
  for (auto& [token, subscriptions] : mouseSubscriptions_) {
    auto it =
        std::ranges::find_if(subscriptions.begin(), subscriptions.end(),
  ...
  auto moveIt = std::ranges::find_if(
      mouseMoveSubscriptions_.begin(), mouseMoveSubscriptions_.end(),
  ...
  auto scrollIt = std::ranges::find_if(
      mouseScrollSubscriptions_.begin(), mouseScrollSubscriptions_.end(),
