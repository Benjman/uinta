...
Texture::Texture(GLenum target, i32 width, i32 height, i32 channels, i32 filter, i32 wrap, const OpenGLApi* gl) noexcept
    : target_(target),
      width_(width),
      height_(height),
      channels_(channels),
...
Texture::Texture(GLenum target, i32 width, i32 height, const TextureFormat& format, i32 filterMin, i32 filterMag,
...
void Texture::setStorage(i32 width, i32 height) noexcept {
  ...
  if (format_.immutable) {
    gl_->texStorage2D(target_, static_cast<GLsizei>(format_.levels), format_.internalFormat, width_, height_);
  } else {
    gl_->texImage2D(target_, 0, static_cast<GLint>(format_.internalFormat), width_, height_, 0, format_.pixelFormat,
                    format_.pixelType, nullptr);
  }
}
...
Status Texture::fromFile(const std::filesystem::path& path, const StbImageApi* imageApi) noexcept {
  ...
  u8* data = imageApi->load(resolvedPath.value().c_str(), &width_, &height_, &channels_, 0);

  TextureGuard guard(this);
  gl_->texImage2D(target_, 0, GL_RGB, width_, height_, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
