...
ViewportManager::ViewportManager(Engine* engine, AppConfig* appConfig) noexcept : appConfig_(appConfig) {
  assert(engine);
  assert(appConfig_);

  auto projStr = appConfig_->getString("viewport.projection").value_or("Perspective");
  ...
  config_.far = appConfig_->getFloat("viewport.far").value_or(100.0f);
  config_.orthoSize = appConfig_->getFloat("viewport.orthoSize").value_or(5.0f);

  engine->dispatchers()->addListener<EngineEvent::ViewportSizeChange>([this](const auto& event) {
...
ViewportManager::~ViewportManager() noexcept {
  if (auto status = appConfig_->setFloat("viewport.far", config_.far); !status.ok()) {
    LOG(WARNING) << "Failed to cache viewport.far: " << status.message();
  }
  if (auto status = appConfig_->setFloat("viewport.fov", config_.fov); !status.ok()) {
    LOG(WARNING) << "Failed to cache viewport.fov: " << status.message();
  }
  if (auto status = appConfig_->setFloat("viewport.near", config_.near); !status.ok()) {
    LOG(WARNING) << "Failed to cache viewport.near: " << status.message();
  }
  if (auto status = appConfig_->setString("viewport.projection", toString(config_.projection)); !status.ok()) {
    LOG(WARNING) << "Failed to cache viewport.projection: " << status.message();
  }
  if (auto status = appConfig_->setFloat("viewport.orthoSize", config_.orthoSize); !status.ok()) {
...
glm::mat4 ViewportManager::projectionMatrix() const noexcept {
  switch (config_.projection) {
    case ProjectionType::Perspective:
      return glm::perspective(glm::radians(config_.fov), aspect_, config_.near, config_.far);
    case ProjectionType::Orthographic: {
      f32 halfWidth = config_.orthoSize * aspect_;
      f32 halfHeight = config_.orthoSize;
      return glm::ortho(-halfWidth, halfWidth, -halfHeight, halfHeight, config_.near, config_.far);
    }
    default:
      return {1.0f};
  }
...
void ViewportManager::dispatchProjectionUpdate() noexcept {
  dispatchers_.dispatch<ViewportEvent::ProjectionMatrixUpdated>(ProjectionMatrixUpdateEvent{
      .viewport = this,
