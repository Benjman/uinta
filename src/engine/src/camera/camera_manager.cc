...
void CameraManager::update(time_t delta) noexcept {
  ...
  comparator_ = camera_;
  dispatchers_.dispatch<CameraEvent::ViewMatrixUpdated>(ViewMatrixUpdateEvent(&camera_, camera_.viewMatrix()));
}
...
CameraInputDeltas CameraManager::computeDeltas(time_t delta) const noexcept {
  ...
  if (config.flags.isMouseEnabled() && input_->isMouseDown(kb.angleM) && (input_->cursordx() != 0.0f)) {
  ...
  if (config.flags.isMouseEnabled() && input_->isMouseDown(kb.pitchM) && (input_->cursordy() != 0.0f)) {
  ...
  if (config.flags.isKeyboardEnabled() && input_->isKeyDown(kb.forwardK)) {
    deltas.translation += glm::normalize(WorldHorizontal * getForward(config.pitch, config.angle)) * kb.trnslSpdK;
  }
  ...
  if (config.flags.isKeyboardEnabled() && input_->isKeyDown(kb.leftK)) {
    deltas.translation += -glm::normalize(getRight(config.angle) * WorldHorizontal) * kb.trnslSpdK;
  }
  ...
  if (config.flags.isMouseEnabled() && input_->isMouseDown(kb.trnslM) &&
    ...
    deltas.translation -=
        (glm::normalize(WorldHorizontal * getRight(config.angle)) * input_->cursordx() -
         glm::normalize(WorldHorizontal * getForward(config.pitch, config.angle)) * input_->cursordy()) *
        kb.trnslSpdM;
  }
