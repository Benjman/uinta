...
void Camera::update(time_t delta, const CameraInputDeltas& deltas) noexcept {
  ...
  if (!config_.flags.isPitchLock()) {
    ...
    if (config_.flags.isPitchLimit()) {
      config_.pitch = std::clamp(config_.pitch.target(), config_.pitchMin, config_.pitchMax);
    }
  ...
  if (!config_.flags.isDistLock()) {
    ...
    if (config_.flags.isDistLimit()) {
      config_.dist = std::clamp(config_.dist.target(), config_.dstMin, config_.dstMax);
    }
  ...
  position_ += static_cast<glm::vec3>(config_.target) + config_.vertOff * WorldUp;
}
...
glm::mat4 Camera::viewMatrix() const noexcept {
  auto result = glm::rotate(glm::mat4(1), glm::radians(config_.pitch.current()), WorldRight);
