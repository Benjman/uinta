...
struct quad final {
    static const inline char MIN_CELL_SIZE = 2;
    ...
    char entityStoreSize;
    char entityCount;

    quad();
    quad(const vec2 &topLeftBounds, const vec2 &bottomRightBounds, const char minCellSize = MIN_CELL_SIZE);
    ~quad();
