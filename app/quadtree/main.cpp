...
struct QuadtreeRunner final : GlfwRunner {
  ...
  void doTick(const RunnerState &state) override {
    ...
    float sin_inner = 0.25 * (std::sin(state.runtime * 0.33) + 2);
    float cos_outer = 0.50 * (std::cos(state.runtime * 0.25) + 1);
    float sin_outer = 0.50 * (std::sin(state.runtime * 0.10) + 1);
    ...
    GLfloat squareVertices[] = {
        ...
        squareOuterNorm.x - squareWidth * 0.5f,  squareOuterNorm.y + squareHeight * 0.5f, squareOuterNorm.x - squareWidth * 0.5f,
        squareOuterNorm.y - squareHeight * 0.5f, squareOuterNorm.x + squareWidth * 0.5f,  squareOuterNorm.y - squareHeight * 0.5f,
