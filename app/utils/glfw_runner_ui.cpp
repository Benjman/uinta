...
bool inputUi(const InputState& input, const Display& display);
bool settings(GlfwRunnerUiResult& result, bool& showGridState);
...
GlfwRunnerUiResult GlfwRunnerUi::updateAndRender(const GlfwRunner& runner) {
  ...
  if (inputUi(runner.state.input, runner.display)) setFlag(GlfwRunnerUiResult::INPUT_HANDLED, true, result.flags);
  if (settings(result, cb_showGrid)) setFlag(GlfwRunnerUiResult::INPUT_HANDLED, true, result.flags);
  ImGui::End();
  ImGui::Render();
...
inline bool camera(const TargetCamera& camera) {
  ...
  hasInput |= ImGui::DragScalar("Dist", ImGuiDataType_Float, (void*)&camera.dist.target, 0.1f, 0, &threeSixty, "%+.2f");
  hasInput |= ImGui::DragScalar("Pitch", ImGuiDataType_Float, (void*)&camera.pitch.target, 0.1f, 0, &threeSixty, "%+.2f");
  hasInput |= ImGui::DragScalar("Angle", ImGuiDataType_Float, (void*)&camera.angle.target, 0.1f, 0, &threeSixty, "%+.2f");
...
inline bool settings(GlfwRunnerUiResult& result, bool& showGridState) {
  if (ImGui::CollapsingHeader("Settings")) {
    if (ImGui::Checkbox("Show grid", &showGridState)) setFlag(GlfwRunnerUiResult::SHOW_GRID_FLAG, true, result.flags);
  }
  return false;
}

inline bool inputUi(const InputState& input, const Display& display) {
  if (!ImGui::CollapsingHeader("Input")) return false;

  ImGui::Text("flags      %6i", input.flags);
  ImGui::Text("pos  (%4.0f, %4.0f)", input.cursorx, input.cursory);
  ImGui::Text("ndc  (%0.2f, %0.2f)", input.cursorx / (float)display.width, input.cursory / (float)display.height);
  // ImGui::Text("view  (%0.2f, %0.2f)", ); // TODO
  // ImGui::Text("world (%0.2f, %0.2f)", ); // TODO

  ImGui::Separator();

  if (ImGui::BeginTable("mouseUi", 2)) {
