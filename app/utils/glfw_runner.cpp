...
void uinta::createGLFWWindow(GlfwRunner& runner) {
  ...
  if (!glfwInit()) return SPDLOG_LOGGER_ERROR(logger, "Failed to initialize GLFW!");
  ...
  if (runner.window == NULL) return SPDLOG_LOGGER_ERROR(logger, "Failed to create GLFW window!");

  glfwSetWindowUserPointer(runner.window, &runner);
  glfwMakeContextCurrent(runner.window);

  SPDLOG_LOGGER_INFO(logger, "Loading GLAD...");
  if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) return SPDLOG_LOGGER_ERROR(logger, "Failed to load GLAD!");
